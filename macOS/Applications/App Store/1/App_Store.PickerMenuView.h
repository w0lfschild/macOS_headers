//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "App_Store.ClickableView.h"

#import "NSMenuDelegate-Protocol.h"

@class NSImageView, _TtC9App_Store20DynamicTypeTextField;

@interface App_Store.PickerMenuView : App_Store.ClickableView <NSMenuDelegate>
{
    // Error parsing type: , name: titleView
    // Error parsing type: , name: selectionImage
    // Error parsing type: , name: options
    // Error parsing type: , name: selectedOption
    // Error parsing type: , name: optionMenu
    // Error parsing type: , name: isImportantForAccessibility
}

+ (double)titleRightMargin;
- (CDUnknownBlockType).cxx_destruct;
- (id)initWithFrame:(struct CGRect)arg1;
- (BOOL)accessibilityPerformPress;
- (id)accessibilityChildren;
- (id)accessibilityTitle;
- (id)accessibilityRole;
- (BOOL)accessibilityShouldUseUniqueId;
- (BOOL)isAccessibilityElement;
@property(nonatomic) BOOL isImportantForAccessibility; // @synthesize isImportantForAccessibility;
- (void)menu:(id)arg1 willHighlightItem:(id)arg2;
- (void)showPopUp;
- (void)layout;
- (struct JEMeasurements)measurementsWithFitting:(struct CGSize)arg1 in:(id)arg2;
- (BOOL)isFlipped;
@property(nonatomic, readonly) NSImageView *selectionImage; // @synthesize selectionImage;
@property(nonatomic, readonly) _TtC9App_Store20DynamicTypeTextField *titleView; // @synthesize titleView;
- (id)initWithCoder:(id)arg1;

// Remaining properties
@property(nonatomic, readonly) BOOL flipped;

@end

