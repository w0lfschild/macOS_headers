//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <AppKit/NSViewController.h>

#import "NSTokenFieldCellDelegate-Protocol.h"

@class NSArrayController, NSString, NSTableView;

__attribute__((visibility("hidden")))
@interface AMVariablesViewController : NSViewController <NSTokenFieldCellDelegate>
{
    NSTableView *__tableView;
    NSArrayController *__arrayController;
}

@property __weak NSArrayController *_arrayController; // @synthesize _arrayController=__arrayController;
@property __weak NSTableView *_tableView; // @synthesize _tableView=__tableView;
- (void).cxx_destruct;
- (BOOL)tokenFieldCell:(id)arg1 hasMenuForRepresentedObject:(id)arg2;
- (void)reloadData;
- (BOOL)tableView:(id)arg1 writeRowsWithIndexes:(id)arg2 toPasteboard:(id)arg3;
- (BOOL)tableView:(id)arg1 acceptDrop:(id)arg2 row:(long long)arg3 dropOperation:(unsigned long long)arg4;
- (unsigned long long)tableView:(id)arg1 validateDrop:(id)arg2 proposedRow:(long long)arg3 proposedDropOperation:(unsigned long long)arg4;
- (void)doDoubleClickInTableView:(id)arg1;
- (void)makeNewVariable:(id)arg1;
- (void)selectVariable:(id)arg1;
@property(readonly) __weak NSArrayController *arrayController;
- (void)dealloc;
- (void)awakeFromNib;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

