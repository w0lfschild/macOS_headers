//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "DEDDeviceFBASupport-Protocol.h"
#import "DEDDeviceLike-Protocol.h"
#import "DEDDeviceUISupport-Protocol.h"
#import "FBADiffableObject-Protocol.h"

@class NSImage, NSMutableDictionary, NSString;

@interface FBAGroupedDevice : NSObject <DEDDeviceLike, DEDDeviceFBASupport, DEDDeviceUISupport, FBADiffableObject>
{
    NSMutableDictionary *_devices;
}

+ (id)inspectFBAPairing;
+ (id)deviceDefaults;
+ (id)currentDevicePlatform;
+ (id)currentGroupedDevice;
+ (id)fbaKnownDevices;
+ (id)sortedDevices:(id)arg1;
- (void).cxx_destruct;
@property(retain) NSMutableDictionary *devices; // @synthesize devices=_devices;
- (void)_postDeviceChangeNotification;
- (id)readyDevice;
- (id)bestStringForKey:(id)arg1;
- (void)deviceListChanged:(id)arg1;
- (void)_updateWithDevices:(id)arg1;
- (id)diffableHashWithContext:(id)arg1;
- (BOOL)isVisibleToDED;
- (void)removeFBAPairing;
- (void)updateStoredCopyIfNeeded;
- (void)pairToFBA;
- (id)displayType;
- (id)displayName;
- (BOOL)isFBAPaired;
- (id)imageData;
- (void)fetchIconImageDataForScale:(float)arg1 completionCompletion:(CDUnknownBlockType)arg2;
- (id)deviceDefaults;
- (id)debugDetails;
- (id)hashingKey;
- (id)shortDescription;
- (BOOL)hasCapabilities:(id)arg1;
- (id)serialize;
- (id)productType;
- (id)name;
- (id)deviceClass;
- (id)color;
- (id)build;
- (id)publicLogDescription;
- (BOOL)isCurrentDevice;
- (id)platform;
- (id)identifier;
- (id)deviceDataForSubmissionWithSession:(id)arg1;
- (BOOL)isLikeDEDDevice:(id)arg1;
- (void)addDevice:(id)arg1;
- (id)inferredPlatform;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)mostReliableDevice;
- (id)anyDEDDevice;
- (id)dedSharingDevice;
- (id)dedIDSDevice;
- (BOOL)needsPairing;
- (BOOL)isReady;
- (id)transportsForDisplay;
@property(readonly, copy) NSString *description;
- (BOOL)isEqualToDevice:(id)arg1;
- (BOOL)isEqual:(id)arg1;
- (void)dealloc;
- (id)initWithDevices:(id)arg1;
@property(readonly) unsigned long long hash;
@property(nonatomic, readonly) NSImage *imageForConnectionState;
@property(nonatomic, readonly) NSString *toolTipForConnectionState;
@property(nonatomic, readonly) NSString *typeForDisplayInCell;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly) Class superclass;

@end

