//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "CLgWindowController.h"

#import "MAPopoverDelegate-Protocol.h"
#import "NSToolbarDelegate-Protocol.h"
#import "NSWindowDelegate-Protocol.h"

@class CLgActionBarCustomizationController, CLgActionBarView, CLgDecorationView, CLgMainSplitView, CLgTransportBackgroundView, CLgTransportBarView, CLgTransportCustomizationController, NSMutableDictionary, NSString;

@interface CLgDocwController : CLgWindowController <NSWindowDelegate, NSToolbarDelegate, MAPopoverDelegate>
{
    NSMutableDictionary *toolbarItemViewDict;
    CLgMainSplitView *mainSplitView;
    CLgTransportBarView *transportBarView;
    CLgTransportBackgroundView *transportBackgroundView;
    CLgDecorationView *decorationViewLeft;
    CLgDecorationView *decorationViewRight;
    CLgActionBarView *actionBarView;
    BOOL transportIsAtTop;
    CLgTransportCustomizationController *_transportCustomizationController;
    CLgActionBarCustomizationController *_actionBarCustomizationController;
    BOOL _unhidePlugInWindows;
}

+ (void)dragEditorView:(id)arg1 withEvent:(id)arg2 docwType:(int)arg3 songDocument:(id)arg4;
+ (BOOL)editorTabCanBeTornOfForDocwType:(int)arg1 songDocument:(id)arg2;
+ (void)dragListView:(id)arg1 withEvent:(id)arg2 docwType:(int)arg3;
- (void)calledByWindowClose;
- (void)windowWasShown;
- (void)showInspectors:(BOOL)arg1;
- (void)toggleInspectors;
- (BOOL)inspectorIsVisible;
- (id)inspectorView;
- (void)setUnilibViewsUserData:(id)arg1;
- (void)viewButtonActionForViewBtnID:(int)arg1;
- (void)listsAreaVisibilityChange2;
- (void)editorsAreaVisibilityChange2;
- (void)inspectorVisibilityChange2;
- (void)libraryVisibilityChange2;
- (void)helpVisibilityChange2;
- (void)toolbarVisibilityChange3;
- (void)toolbarVisibilityChange2;
- (id)willSendMouseDownAtPosition:(struct CGPoint)arg1 isOnUnilibViewSelf:(char *)arg2;
- (BOOL)shouldDisplayFocusRing;
- (id)focusedUnilibView;
- (void)firstResponderChanged;
- (id)focusHandlerViewToSelect:(id)arg1 ifFollowing:(BOOL)arg2;
- (void)resetNextKeyViews;
- (BOOL)presentToolContextMenu:(id)arg1 forView:(id)arg2;
- (void)customizeToolWillBegin:(id)arg1;
- (id)actionBarCustomizationController;
- (BOOL)presentTransportContextMenu:(id)arg1 forView:(id)arg2;
- (id)transportContextMenu;
- (void)showTransportButtonAutoPunch;
- (void)customizeTransportWillBegin:(id)arg1;
- (id)transportCustomizationController;
- (void)popoverWillClose:(id)arg1;
- (void)openPopover:(id)arg1 forView:(id)arg2;
- (void)doShowTransportBar:(BOOL)arg1 actionBar:(BOOL)arg2;
- (unsigned long long)transportBarRows;
- (BOOL)transportBarIsVisible;
- (BOOL)transportBarIsPossible;
- (void)setTransportBarIsAtTop:(BOOL)arg1;
- (BOOL)transportBarIsAtTop;
- (id)timeDisplayView;
- (id)transportView;
- (void)setTransportView:(id)arg1;
- (id)actionBarView;
- (void)setActionBarView:(id)arg1;
- (id)transportBackgroundView;
- (void)setTransportBackgroundView:(id)arg1;
- (id)decorationViewRight;
- (id)decorationViewLeft;
- (void)setDecorationViewLeft:(id)arg1 viewRight:(id)arg2;
- (BOOL)mixerAreaVisible;
- (BOOL)smartControlsAreaVisible;
- (BOOL)listsAreaVisible;
- (BOOL)editorAreaVisible;
- (void)toggleEditorArea;
- (BOOL)hideEditorArea;
- (id)browserUnilibView;
- (void)closeLibraryArea;
- (void)closeListsArea;
- (void)toggleListArea:(int)arg1;
- (BOOL)isListAreaVisible:(int)arg1;
- (id)listView:(int)arg1;
- (BOOL)toggleListView:(int)arg1;
- (BOOL)showListView:(int)arg1;
- (BOOL)isListViewVisible:(int)arg1;
- (void)toggleEditorView;
- (BOOL)isEditorViewVisible;
- (id)editorView:(int)arg1;
- (BOOL)toggleEditorView:(int)arg1;
- (BOOL)showEditorView:(int)arg1;
- (BOOL)isEditorViewVisible:(int)arg1;
- (id)unilibDocManager3;
- (BOOL)unilibViewIsVisible:(id)arg1;
- (id)visibleUnilibViews1;
- (id)allUnilibViews1;
- (id)allNotesFocusViews;
- (id)allUnilibViews_includeAll:(BOOL)arg1;
- (id)visibleUnilibViews_includeAll:(BOOL)arg1;
- (struct CGSize)calculateMinimumWindowSize;
- (void)synchronizeWindowTitleWithDocumentName;
- (id)windowTitle;
- (id)unilibViewAtWindowPosition:(struct CGPoint)arg1;
- (id)windowMainView;
- (id)editorsArea;
- (id)listViewForListViewID:(int)arg1;
- (id)inspectorArea;
- (id)mainSplitView;
- (void)setMainSplitView:(id)arg1;
- (void)dealloc;
- (id)initDocwController:(id)arg1;
- (void)quickHelpVisibilityChanged:(id)arg1;
- (void)removeNotifications;
- (void)addNotifications;
- (void)validateActionBarItems;
- (BOOL)validateToolbarItem:(id)arg1;
- (void)toolbarTextAction:(id)arg1;
- (void)toolbarGenericAction:(id)arg1;
- (void)toolbarGenericSegmentedAction:(id)arg1;
- (void)libraryVisibilityChange1;
- (void)listsAreaVisibilityChange1;
- (void)inspectorVisibilityChange1;
- (void)setToolbarState1:(long long)arg1 forToolbarItemID:(int)arg2;
- (void)setOnState:(long long)arg1 forIdentifier:(id)arg2;
- (BOOL)validateMenuItem:(id)arg1;
- (void)toolbarMenuGenericAction:(id)arg1;
- (void)listsMenuGenericAction:(id)arg1;
- (void)displayModeLabelOnlyAction:(id)arg1;
- (void)toolbarGenericActionForIdentifier:(id)arg1;
- (void)menuNeedsUpdate:(id)arg1;
- (int)toolbarItemIDFromItemIdentifier:(id)arg1;
- (id)toolbarItemIdentifierFromItemID:(int)arg1;
- (BOOL)toolbarIsVisible;
- (void)setToolbarIsVisible:(BOOL)arg1;
- (void)setupToolbar;
- (id)toolbar:(id)arg1 itemForItemIdentifier:(id)arg2 willBeInsertedIntoToolbar:(BOOL)arg3;
- (void)toolbarDidRemoveItem:(id)arg1;
- (void)toolbarWillAddItem:(id)arg1;
- (void)setupToolbarItem:(id)arg1 imageName:(id)arg2 label:(id)arg3 tooltip:(id)arg4 menuArray:(id)arg5 menuDelegate:(id)arg6;
- (void)setQuickHelpIdDelayed:(id)arg1;
- (void)setupSegmentedToolbarItem:(id)arg1 label:(id)arg2 segments:(id)arg3;
- (id)quickHelpIdForToolbarItemIdentifier:(id)arg1;
- (id)toolbarDefaultItemIdentifiers:(id)arg1;
- (id)allToolbarDefaultItemIdentifiers;
- (id)toolbarAllowedItemIdentifiers:(id)arg1;
- (id)allToolbarAllowedItemIdentifiers;
- (id)filterAllowedItemIdentifiers:(id)arg1;
- (id)mixedStateButtonActionBarItemIdentifiers;
- (id)toggleButtonActionBarItemIdentifiers;
- (struct CGRect)windowWillUseStandardFrame:(id)arg1 defaultFrame:(struct CGRect)arg2;
- (void)resetFocusedElement:(int)arg1;
- (id)layoutDictForWindowController;
- (void)createDocwViewsForLayout:(int)arg1 docwState:(id)arg2 unilibDocManager:(id)arg3;
- (id)addDocwWindowChildren:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

