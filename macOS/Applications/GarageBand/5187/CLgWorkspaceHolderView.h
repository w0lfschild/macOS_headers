//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "MAWorkspaceHolderView.h"

#import "LgSectionView.h"

@class CLgView, CLgWorkspaceFreezeView, MAButton, NSAttributedString, NSColor, NSImage, NSShadow, NSView, SmartControlsViewController;

@interface CLgWorkspaceHolderView : MAWorkspaceHolderView <LgSectionView>
{
    CLgView *parentView;
    NSView *scaleableView;
    BOOL showsVisualEQ;
    BOOL showsRemixFX;
    BOOL showsMasterFX;
    SmartControlsViewController *scvc;
    NSImage *bgImage;
    NSImage *tolexImage;
    NSShadow *dropShadow;
    NSAttributedString *noPluginsText;
    NSAttributedString *addEQText;
    BOOL showPlaceholderText;
    MAButton *addEQButton;
    CLgWorkspaceFreezeView *freezeView;
    BOOL _userInputDisabled;
    BOOL showsParrotMain;
    BOOL showsParrotDetail;
    BOOL showsMicrobeat;
    NSAttributedString *unsupportedTrack;
    id userInputDisabledTarget;
    SEL userInputDisabledAction;
    NSColor *_backgroundColor;
}

@property(retain) NSColor *backgroundColor; // @synthesize backgroundColor=_backgroundColor;
@property(readonly) BOOL showsMicrobeat; // @synthesize showsMicrobeat;
@property(readonly) BOOL showsParrotDetail; // @synthesize showsParrotDetail;
@property(readonly) BOOL showsParrotMain; // @synthesize showsParrotMain;
@property(readonly) BOOL showsMasterFX; // @synthesize showsMasterFX;
@property(readonly) BOOL showsRemixFX; // @synthesize showsRemixFX;
@property(readonly) BOOL showsVisualEQ; // @synthesize showsVisualEQ;
@property(nonatomic) SEL userInputDisabledAction; // @synthesize userInputDisabledAction;
@property(nonatomic) id userInputDisabledTarget; // @synthesize userInputDisabledTarget;
@property CLgView *parentView; // @synthesize parentView;
@property(retain, nonatomic) NSAttributedString *unsupportedTrack; // @synthesize unsupportedTrack;
- (void)conditionallyShowWorkspace;
- (BOOL)toggleKeyswitches;
- (BOOL)toggleMicrobeat;
- (BOOL)toggleParrotDetail;
- (BOOL)toggleParrotMain;
- (BOOL)toggleMasterFX;
- (BOOL)toggleRemixFX;
- (BOOL)toggleVisualEQ;
@property BOOL displayTolex;
- (void)setKeyswitchesView:(id)arg1;
- (void)setPlugInView:(id)arg1;
- (BOOL)hasPlugInView;
- (void)centerAndScaleScaleableView;
- (void)resizeSubviewsWithOldSize:(struct CGSize)arg1;
- (void)updateFreezeView;
@property(nonatomic) BOOL userInputDisabled; // @synthesize userInputDisabled=_userInputDisabled;
- (void)drawRect:(struct CGRect)arg1;
- (void)viewWillDraw;
- (void)setHideWorkspaceView:(BOOL)arg1 force:(BOOL)arg2;
- (BOOL)acceptsFirstResponder;
@property(readonly) BOOL hasPlayhead;
@property(readonly) BOOL isContents;
@property(readonly) int section;
- (void)dealloc;
- (id)initWithFrame:(struct CGRect)arg1 scViewController:(id)arg2;

// Remaining properties
@property(readonly) BOOL isDragging;

@end

