//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <Foundation/NSObject.h>

#import "GrowlApplicationBridgeDelegate-Protocol.h"

@class NSString;

@interface growlController : NSObject <GrowlApplicationBridgeDelegate>
{
    BOOL growlRunning;
}

- (void)passwordNeeded:(id)arg1;
- (void)compressionFailed:(id)arg1;
- (void)compressionComplete:(id)arg1;
- (void)extractionFailed:(id)arg1;
- (void)extractionComplete:(id)arg1;
- (void)betaService:(id)arg1;
- (id)registrationDictionaryForGrowl;
- (void)countNotification;
- (void)growlNotificationTimedOut:(id)arg1;
- (void)growlNotificationWasClicked:(id)arg1;
- (void)dealloc;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

