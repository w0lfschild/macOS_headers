//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "KNAnimationEffect.h"

#import "KNTransitionCAAnimator-Protocol.h"
#import "KNTransitionMetalAnimator-Protocol.h"
#import "KNTransitionOpenGLAnimator-Protocol.h"

@class CAMediaTimingFunction, KNMotionBlurAnimationPluginWrapper, NSString;
@protocol MTLRenderPipelineState, TSDMTLDataBuffer;

@interface KNTransitionDissolve : KNAnimationEffect <KNTransitionCAAnimator, KNTransitionOpenGLAnimator, KNTransitionMetalAnimator>
{
    KNMotionBlurAnimationPluginWrapper *_motionBlurWrapper;
    CAMediaTimingFunction *_timingFunction;
    id <TSDMTLDataBuffer> _dataBuffer;
    id <MTLRenderPipelineState> _pipelineState;
    // Error parsing type: {?="MVPMatrix"{?="columns"[4]}}, name: _vertexUniforms
    // Error parsing type: {?="mvpMatrix"{?="columns"[4]}"previousMVPMatrix"{?="columns"[4]}"velocityScale""textureMatrix"{?="columns"[3]}"opacity"f"blendPercent"f}, name: _fragmentUniforms
}

+ (long long)rendererTypeForAnimationContext:(id)arg1;
+ (id)thumbnailImageNameForType:(long long)arg1;
+ (id)defaultAttributes;
+ (void)fillLocalizedDirectionMenu:(id)arg1 forType:(long long)arg2;
+ (unsigned long long)directionType;
+ (id)localizedMenuString:(long long)arg1;
+ (id)supportedTypes;
+ (id)animationFilter;
+ (long long)animationCategory;
+ (id)animationName;
- (void).cxx_destruct;
- (void)animationDidEndWithContext:(id)arg1;
- (void)renderFrameWithContext:(id)arg1;
- (void)animationWillBeginWithContext:(id)arg1;
- (void)addAnimationsTo:(id)arg1 context:(id)arg2;
- (id)initWithAnimationContext:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

