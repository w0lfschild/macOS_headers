//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "TSWPStorageSpellChecker.h"

@class NSHashTable;

@interface TSWPMacStorageSpellChecker : TSWPStorageSpellChecker
{
    long long _inFlight;
    long long _sequenceNumber;
    struct _NSRange _rangeInFlight;
    unsigned long long _storageChangeCount;
    NSHashTable *_operationsInFlight;
}

+ (void)p_inputLanguageDidChangeNotification:(id)arg1;
+ (id)spellCheckerForStorage:(id)arg1 selectionPath:(id)arg2 orSearchCanvasDelegate:(id)arg3;
+ (void)initialize;
@property(nonatomic) struct _NSRange rangeInFlight; // @synthesize rangeInFlight=_rangeInFlight;
@property(readonly, nonatomic) long long sequenceNumber; // @synthesize sequenceNumber=_sequenceNumber;
@property(nonatomic) long long inFlight; // @synthesize inFlight=_inFlight;
- (void).cxx_destruct;
- (BOOL)hasLearnedWordWithRange:(struct _NSRange)arg1;
- (long long)p_spellCheckerDocumentTag;
- (id)p_languageCode;
- (id)textCheckingResultsForRange:(struct _NSRange)arg1;
- (struct _NSRange)rangeOfNumericalSuffixPrecedingCharIndex:(unsigned long long)arg1;
- (void)p_addGrammarChecking:(id)arg1 text:(id)arg2 ifValidForRange:(struct _NSRange)arg3 results:(id)arg4;
- (BOOL)i_addSpellingAndGrammarMarksInRange:(struct _NSRange)arg1 spellingResults:(id)arg2 grammarResults:(id)arg3 sync:(BOOL)arg4;
- (BOOL)isBusy;
- (BOOL)hasValidResultsForWordAtCharIndex:(unsigned long long)arg1 outRange:(out struct _NSRange *)arg2;
- (void)findErrorsInRange:(struct _NSRange)arg1 onHit:(CDUnknownBlockType)arg2 stop:(char *)arg3;
- (id)p_textCheckingResultForWordAtCharIndex:(unsigned long long)arg1 types:(unsigned long long)arg2 outRange:(struct _NSRange *)arg3;
- (void)didRejectReplacement:(id)arg1 forRange:(struct _NSRange)arg2;
- (void)willRevertReplacementRange:(struct _NSRange)arg1 forWord:(id)arg2;
- (id)p_substringWithoutDeletionsWithRange:(struct _NSRange)arg1;
- (void)storage:(id)arg1 didChangeRange:(struct _NSRange)arg2 delta:(long long)arg3 broadcastKind:(int)arg4;
- (id)grammarResultForCharIndex:(unsigned long long)arg1 range:(out struct _NSRange *)arg2;
- (struct _NSRange)firstErrorInRange:(struct _NSRange)arg1;
- (BOOL)hasReplacementForCharIndex:(unsigned long long)arg1 type:(unsigned long long)arg2 outRange:(struct _NSRange *)arg3 outString:(id *)arg4;
- (id)correctionStringForWordAtCharIndex:(unsigned long long)arg1;
- (struct _NSRange)rangeOfAutocorrectCandidateWordAtCharIndex:(unsigned long long)arg1;
- (id)p_autoCorrectionForWordAtCharIndex:(unsigned long long)arg1 outRange:(struct _NSRange *)arg2;
- (void)dealloc;
- (id)initWithStorage:(id)arg1 selectionPath:(id)arg2 orSearchCanvasDelegate:(id)arg3;
- (void)p_resetSpellCheckingForNotification:(id)arg1;
- (void)p_spellCheckerDidChangeLanguageNotification:(id)arg1;
- (void)p_spellCheckerDidUnlearnWordNotification:(id)arg1;
- (void)p_spellCheckerDidLearnWordNotification:(id)arg1;
- (id)suggestionsForRange:(struct _NSRange)arg1;

@end

