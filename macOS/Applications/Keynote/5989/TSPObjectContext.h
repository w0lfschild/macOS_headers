...
@class NSData, NSHashTable, NSMapTable, NSMutableArray, NSMutableSet, NSProgress, NSRecursiveLock, NSSet, NSString, NSURL, NSUUID, SFUCryptoKey, TSPCancellationState, TSPComponentManager, TSPDataDownloadManager, TSPDataManager, TSPDocumentDataWatcher, TSPDocumentMetadata, TSPDocumentProperties, TSPDocumentRevision, TSPDocumentSaveOperationState, TSPObject, TSPObjectContainer, TSPObjectUUIDMap, TSPPackage, TSPPackageWriteCoordinator, TSPRegistry, TSPResourceContext, TSPSupportManager, TSPSupportMetadata, TSUTemporaryDirectory;
...
@interface TSPObjectContext : NSObject <TSPFileCoordinatorDelegate, TSPLazyReferenceDelegate, TSPObjectDelegate, TSPSupportDirectoryDelegate, TSPPassphraseConsumer>
{
    ...
    // Error parsing type: Ac, name: _didClose
    unsigned int _mode;
...
+ (id)releaseQueue;
+ (BOOL)alwaysSaveAsDirectoryPackage;
+ (BOOL)shouldSaveAsPackageToURL:(id)arg1 serviceType:(unsigned long long)arg2 sharingState:(id)arg3 preferredPackageType:(long long)arg4;
+ (BOOL)userCanChoosePackageType:(id)arg1 serviceType:(unsigned long long)arg2 sharingState:(id)arg3;
+ (BOOL)remoteVersionBrowsingSupportedByService:(unsigned long long)arg1;
+ (BOOL)packageFormatSupportedByService:(unsigned long long)arg1;
+ (long long)packageTypeForSavingToCollaborativeCacheWithServiceType:(unsigned long long)arg1;
...
- (void)attemptMaterializingRemoteDataIfNeeded;
