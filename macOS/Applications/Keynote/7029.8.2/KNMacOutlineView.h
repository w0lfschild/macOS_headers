//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AppKit/NSOutlineView.h>

@class KNMacOutlineDropIndicatorView, KNMacOutlineViewController, KNMacSlideContextMenuController, NSIndexSet;
@protocol KNMacOutlineViewDelegate;

@interface KNMacOutlineView : NSOutlineView
{
    KNMacSlideContextMenuController *_contextMenuController;
    KNMacOutlineDropIndicatorView *_dropIndicatorView;
    long long _mouseDownRow;
    BOOL _drawSelectionOnContextMenuRow;
    BOOL _isSelectionDragActive;
    BOOL _blockSelectionChanges;
    BOOL _disableHighlightDrawing;
    BOOL _usesMetricsAppearance;
    BOOL _showsDropIndicator;
    KNMacOutlineViewController *_viewController;
    NSIndexSet *_draggedIndexes;
    long long _contextMenuRow;
    double _slideNumberWidthForDropTargetPositioning;
    long long _dropIndicatorRow;
    long long _dropIndicatorLevel;
}

- (void).cxx_destruct;
@property(nonatomic) long long dropIndicatorLevel; // @synthesize dropIndicatorLevel=_dropIndicatorLevel;
@property(nonatomic) long long dropIndicatorRow; // @synthesize dropIndicatorRow=_dropIndicatorRow;
@property(nonatomic) BOOL showsDropIndicator; // @synthesize showsDropIndicator=_showsDropIndicator;
@property(nonatomic) BOOL disableHighlightDrawing; // @synthesize disableHighlightDrawing=_disableHighlightDrawing;
@property(nonatomic) double slideNumberWidthForDropTargetPositioning; // @synthesize slideNumberWidthForDropTargetPositioning=_slideNumberWidthForDropTargetPositioning;
@property(readonly, nonatomic) long long contextMenuRow; // @synthesize contextMenuRow=_contextMenuRow;
@property(readonly, nonatomic) NSIndexSet *draggedIndexes; // @synthesize draggedIndexes=_draggedIndexes;
@property(nonatomic) __weak KNMacOutlineViewController *viewController; // @synthesize viewController=_viewController;
- (void)selectRowsFromRow:(long long)arg1 toPoint:(struct CGPoint)arg2;
- (void)mouseUp:(id)arg1;
- (void)mouseDragged:(id)arg1;
- (void)mouseDown:(id)arg1;
- (void)deleteBackward:(id)arg1;
- (void)deleteForward:(id)arg1;
- (void)outdentParagraphLevel:(id)arg1;
- (void)insertBacktab:(id)arg1;
- (void)indentParagraphLevel:(id)arg1;
- (void)insertTab:(id)arg1;
- (void)insertNewline:(id)arg1;
- (void)insertText:(id)arg1;
- (void)moveRight:(id)arg1;
- (void)moveLeft:(id)arg1;
- (id)p_shiftArrowKeyEventWithType:(unsigned long long)arg1 keyChar:(unsigned short)arg2 keyCode:(unsigned short)arg3;
- (void)keyDown:(id)arg1;
- (void)removePhonetics:(id)arg1;
- (void)phonetics:(id)arg1;
- (void)editMasterForSlideNodeInContext:(id)arg1;
- (void)skipSlideNodesInContext:(id)arg1;
- (void)skipSelectedSlideNodes:(id)arg1;
- (void)collapseAll:(id)arg1;
- (void)collapse:(id)arg1;
- (void)expandAll:(id)arg1;
- (void)expand:(id)arg1;
- (void)duplicateSlideNodesInContext:(id)arg1;
- (void)duplicate:(id)arg1;
- (void)deleteSlideNodesInContext:(id)arg1;
- (void)delete:(id)arg1;
- (void)pasteAfterSlideNodesInContext:(id)arg1;
- (void)paste:(id)arg1;
- (void)cutSlideNodesInContext:(id)arg1;
- (void)cut:(id)arg1;
- (void)copySlideNodesInContext:(id)arg1;
- (void)copy:(id)arg1;
- (void)insertNewSlideNodeAfterSlideNodesInContext:(id)arg1;
- (void)insertNewSlideNode:(id)arg1;
- (BOOL)validateUserInterfaceItem:(id)arg1;
- (id)menuForEvent:(id)arg1;
- (void)p_contextMenuDidEndTracking;
- (id)contextMenuController;
- (BOOL)validateProposedFirstResponder:(id)arg1 forEvent:(id)arg2;
- (void)p_updateDropIndicatorWithRow:(long long)arg1 level:(long long)arg2;
@property(readonly, nonatomic) KNMacOutlineDropIndicatorView *dropIndicatorView;
- (struct CGRect)p_rectForDrawingBackgroundOnCell:(id)arg1 withRowRect:(struct CGRect)arg2;
- (void)p_drawSelectionBackgroundUsingMetricsAppearanceInRowRect:(struct CGRect)arg1 clipRect:(struct CGRect)arg2 fillColor:(id)arg3 roundTop:(BOOL)arg4 roundBottom:(BOOL)arg5;
- (void)p_drawSelectionBackgroundInRowRect:(struct CGRect)arg1 clipRect:(struct CGRect)arg2 fillColor:(id)arg3 strokeColor:(id)arg4 drawTop:(BOOL)arg5 drawBottom:(BOOL)arg6 context:(id)arg7;
- (void)drawBackgroundInClipRect:(struct CGRect)arg1;
- (long long)p_dropLevelAtPoint:(struct CGPoint)arg1;
- (void)p_drawView:(id)arg1 withCellFrameUnion:(struct CGRect)arg2 inContext:(id)arg3 rowRect:(struct CGRect)arg4 yOffset:(double)arg5 roundTop:(BOOL)arg6 roundBottom:(BOOL)arg7;
- (id)p_dragImageForRowsWithIndexes:(id)arg1 tableColumns:(id)arg2 event:(id)arg3 offset:(struct CGPoint *)arg4;
- (id)dragImageForRowsWithIndexes:(id)arg1 tableColumns:(id)arg2 event:(id)arg3 offset:(struct CGPoint *)arg4;
- (BOOL)performDragOperation:(id)arg1;
- (void)draggingExited:(id)arg1;
- (void)draggingEnded:(id)arg1;
- (unsigned long long)draggingUpdated:(id)arg1;
- (unsigned long long)draggingEntered:(id)arg1;
- (void)updateSelectedCellTextColorForSelectionState;
- (void)selectRowIndexes:(id)arg1 byExtendingSelection:(BOOL)arg2;
- (void)setSelectionHighlightStyle:(long long)arg1;
- (void)setIntercellSpacing:(struct CGSize)arg1;
- (void)noteHeightOfRowsWithIndexesChanged:(id)arg1;
- (struct CGRect)frameOfOutlineCellAtRow:(long long)arg1;
- (BOOL)acceptsFirstMouse:(id)arg1;
- (BOOL)resignFirstResponder;
- (BOOL)becomeFirstResponder;
- (void)awakeFromNib;
- (void)p_windowKeyStateChanged:(id)arg1;
- (void)dealloc;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;
- (void)p_commonInit;

// Remaining properties
@property __weak id <KNMacOutlineViewDelegate> delegate; // @dynamic delegate;

@end

