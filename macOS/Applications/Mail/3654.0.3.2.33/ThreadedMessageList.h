//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "ThreadSupportingMessageList-Protocol.h"

@class ManagedList, NSArray, NSMutableIndexSet, NSString;
@protocol ThreadedMessageListDelegate;

@interface ThreadedMessageList : NSObject <ThreadSupportingMessageList>
{
    NSMutableIndexSet *_openThreadIndexes;
    id <ThreadedMessageListDelegate> _delegate;
    ManagedList *_threadList;
    unsigned long long _totalMessageCount;
}

+ (id)log;
- (void).cxx_destruct;
@property(nonatomic) unsigned long long totalMessageCount; // @synthesize totalMessageCount=_totalMessageCount;
@property(readonly, nonatomic) ManagedList *threadList; // @synthesize threadList=_threadList;
@property(readonly, nonatomic) __weak id <ThreadedMessageListDelegate> delegate; // @synthesize delegate=_delegate;
- (id)_indexesReversed:(id)arg1 acrossLength:(unsigned long long)arg2;
- (unsigned long long)_childCountForThread:(id)arg1;
- (id)_indexesAdjustedByOpenThreads:(id)arg1;
- (unsigned long long)_indexAdjustedByOpenThreads:(unsigned long long)arg1;
- (unsigned long long)adjustedIndexOfMessage:(id)arg1 adjustForChildren:(BOOL)arg2;
- (BOOL)_threadChildrenOrderedAscending;
@property(readonly, copy, nonatomic) NSArray *unrolledObjects;
- (id)_deletedThreadsFromAdjustingThreads:(id)arg1 messageRemovals:(id)arg2 messageMoves:(id)arg3 animate:(BOOL)arg4;
- (void)removeMessages:(id)arg1 andReplaceMessages:(id)arg2 animate:(BOOL)arg3;
- (void)_shiftOpenThreadIndexesByAddedIndexes:(id)arg1;
- (id)_mergeThreads:(id)arg1;
- (unsigned long long)_mergeThread:(id)arg1;
- (unsigned long long)_removeThread:(id)arg1;
- (id)_threadChangesFromAddingMessages:(id)arg1 primary:(BOOL)arg2 newThreads:(id)arg3 alteredThreads:(id)arg4;
- (void)addPrimaryMessages:(id)arg1 secondaryMessages:(id)arg2;
- (void)updateMessages:(id)arg1;
- (struct _NSRange)_closeThread:(id)arg1 atIndex:(unsigned long long)arg2;
- (void)closeAllThreads;
- (void)closeThreads:(id)arg1;
- (struct _NSRange)_openThread:(id)arg1 atIndex:(unsigned long long)arg2;
- (void)openAllThreads;
- (void)_adjustAdjacentMessagesToThread:(id)arg1 atIndex:(unsigned long long)arg2;
- (void)openThreads:(id)arg1;
- (void)invert;
- (void)applyFilter;
- (void)resort;
- (id)init;
- (id)initWithDelegate:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

