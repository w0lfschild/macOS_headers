//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AppKit/NSViewController.h>

#import "CNContactPickerSectionDelegate-Protocol.h"

@class MSPQuery, MSPQuerySource, NSArray, NSArrayController, NSButton, NSString, NSTableView;

__attribute__((visibility("hidden")))
@interface NVBookmarksViewController : NSViewController <CNContactPickerSectionDelegate>
{
    BOOL _altMode;
    unsigned long long _type;
    MSPQuerySource *_favoritesSource;
    MSPQuery *_favorites;
    MSPQuerySource *_historySource;
    MSPQuery *_history;
    NSArray *_observedEditableBookmarks;
    NSTableView *_tableView;
    NSArrayController *_arrayController;
    NSButton *_bottomActionButton;
}

- (void).cxx_destruct;
@property(retain, nonatomic) NSButton *bottomActionButton; // @synthesize bottomActionButton=_bottomActionButton;
@property(retain, nonatomic) NSArrayController *arrayController; // @synthesize arrayController=_arrayController;
@property(retain, nonatomic) NSTableView *tableView; // @synthesize tableView=_tableView;
- (void)tableView:(id)arg1 draggingSession:(id)arg2 endedAtPoint:(struct CGPoint)arg3 operation:(unsigned long long)arg4;
- (BOOL)contactPickerSection:(id)arg1 acceptDrop:(id)arg2;
- (unsigned long long)contactPickerSection:(id)arg1 validateDrop:(id)arg2;
- (BOOL)tableView:(id)arg1 acceptDrop:(id)arg2 row:(long long)arg3 dropOperation:(unsigned long long)arg4;
- (unsigned long long)tableView:(id)arg1 validateDrop:(id)arg2 proposedRow:(long long)arg3 proposedDropOperation:(unsigned long long)arg4;
- (id)tableView:(id)arg1 pasteboardWriterForRow:(long long)arg2;
- (id)tableView:(id)arg1 viewForTableColumn:(id)arg2 row:(long long)arg3;
- (double)tableView:(id)arg1 heightOfRow:(long long)arg2;
- (id)tableView:(id)arg1 identifierForRow:(long long)arg2;
- (id)tableView:(id)arg1 rowViewForRow:(long long)arg2;
- (BOOL)tableView:(id)arg1 shouldSelectRow:(long long)arg2;
- (void)deleteElementAction:(id)arg1;
- (void)bottomActionButtonClicked:(id)arg1;
- (void)cleanUpState;
- (void)updateDeleteButtonsState:(BOOL)arg1;
- (void)updateBottomActionButtonTitle;
- (void)updateTableState;
@property(nonatomic) BOOL altMode;
- (void)clearEditMode:(id)arg1;
- (void)setFilter:(id)arg1;
- (void)bookmarkClicked:(id)arg1;
- (void)showPlaceOrTrip:(id)arg1;
- (id)_allElements;
- (void)viewWillDisappear;
- (void)viewDidLoad;
- (void)loadView;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;
- (void)_restartObservingEditableBookmarks;
- (void)_favoritesDidChange;
- (void)_stopObservingEditableBookmarks;
- (id)nibBundle;
- (id)nibName;
- (id)nibNameForType:(unsigned long long)arg1;
- (void)dealloc;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2 type:(unsigned long long)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

