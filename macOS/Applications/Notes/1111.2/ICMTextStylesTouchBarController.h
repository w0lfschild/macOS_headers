//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "ICMAppTouchBarController.h"

#import "ICMTextStylesTouchBarDelegate-Protocol.h"
#import "NSTouchBarDelegate-Protocol.h"

@class ICMTextStylesTouchBar, ICMacBaseTextView, NSArray, NSButton, NSCandidateListTouchBarItem, NSCustomTouchBarItem, NSIndexSet, NSPopoverTouchBarItem, NSSet, NSStackView, NSString, NSTouchBar, NSView, NSWindowController;

@interface ICMTextStylesTouchBarController : ICMAppTouchBarController <ICMTextStylesTouchBarDelegate, NSTouchBarDelegate>
{
    NSSet *_textViewProvidedIdentifiers;
    ICMacBaseTextView *_textView;
    NSIndexSet *_manualSelectedStyles;
    id _dfrBIUEnabledBindingObject;
    id _canStyleTextEnabledBindingObject;
    id _canIndentEnabledBindingObject;
    id _canOutdentEnabledBindingObject;
    id _canToggleTodoEnabledBindingObject;
    NSButton *_indentButton;
    NSButton *_outdentButton;
    ICMTextStylesTouchBar *_stylesTouchBar;
    NSStackView *_indentStackView;
    NSCustomTouchBarItem *_addNoteBarItem;
    NSPopoverTouchBarItem *_stylesBarItem;
    NSCustomTouchBarItem *_checklistBarItem;
    NSCustomTouchBarItem *_indentTextBarItem;
    NSCandidateListTouchBarItem *_candidateBarItem;
    NSTouchBar *_editingTouchBar;
    NSView *_textViewBIUButton;
}

- (void).cxx_destruct;
@property(retain, nonatomic) NSView *textViewBIUButton; // @synthesize textViewBIUButton=_textViewBIUButton;
@property(retain, nonatomic) NSTouchBar *editingTouchBar; // @synthesize editingTouchBar=_editingTouchBar;
@property(retain, nonatomic) NSCandidateListTouchBarItem *candidateBarItem; // @synthesize candidateBarItem=_candidateBarItem;
@property(retain, nonatomic) NSCustomTouchBarItem *indentTextBarItem; // @synthesize indentTextBarItem=_indentTextBarItem;
@property(retain, nonatomic) NSCustomTouchBarItem *checklistBarItem; // @synthesize checklistBarItem=_checklistBarItem;
@property(retain, nonatomic) NSPopoverTouchBarItem *stylesBarItem; // @synthesize stylesBarItem=_stylesBarItem;
@property(retain, nonatomic) NSCustomTouchBarItem *addNoteBarItem; // @synthesize addNoteBarItem=_addNoteBarItem;
@property(retain, nonatomic) NSStackView *indentStackView; // @synthesize indentStackView=_indentStackView;
@property(retain, nonatomic) ICMTextStylesTouchBar *stylesTouchBar; // @synthesize stylesTouchBar=_stylesTouchBar;
@property(retain, nonatomic) NSButton *outdentButton; // @synthesize outdentButton=_outdentButton;
@property(retain, nonatomic) NSButton *indentButton; // @synthesize indentButton=_indentButton;
@property(readonly, nonatomic) id canToggleTodoEnabledBindingObject; // @synthesize canToggleTodoEnabledBindingObject=_canToggleTodoEnabledBindingObject;
@property(readonly, nonatomic) id canOutdentEnabledBindingObject; // @synthesize canOutdentEnabledBindingObject=_canOutdentEnabledBindingObject;
@property(readonly, nonatomic) id canIndentEnabledBindingObject; // @synthesize canIndentEnabledBindingObject=_canIndentEnabledBindingObject;
@property(readonly, nonatomic) id canStyleTextEnabledBindingObject; // @synthesize canStyleTextEnabledBindingObject=_canStyleTextEnabledBindingObject;
@property(readonly, nonatomic) id dfrBIUEnabledBindingObject; // @synthesize dfrBIUEnabledBindingObject=_dfrBIUEnabledBindingObject;
@property(retain, nonatomic) NSIndexSet *manualSelectedStyles; // @synthesize manualSelectedStyles=_manualSelectedStyles;
@property(nonatomic) __weak ICMacBaseTextView *textView; // @synthesize textView=_textView;
@property(readonly, nonatomic) NSSet *textViewProvidedIdentifiers; // @synthesize textViewProvidedIdentifiers=_textViewProvidedIdentifiers;
- (void)textStylesTouchBar:(id)arg1 didSelectStyle:(id)arg2 sender:(id)arg3;
@property(readonly, nonatomic) NSArray *customizationRequiredItemIdentifiers;
@property(readonly, nonatomic) NSArray *customizationAllowedItemIdentifiers;
@property(readonly, nonatomic) NSArray *defaultItemIdentifiers;
@property(readonly, nonatomic) NSString *customizationIdentifier;
- (void)updateChecklistButtonAccessibilityValue;
- (void)dismissStylePopoverIfNecessary;
- (void)updateTextTouchBarItems;
- (void)updateStyleButtons;
- (void)updateStyleButtonsIfNecessary;
- (void)styleCollapsedButtonPressed:(id)arg1;
- (void)handleStyleButtonPressed:(id)arg1;
- (void)styleButtonPressed:(id)arg1;
- (void)addNoteButtonPressed:(id)arg1;
- (void)checklistButtonPressed:(id)arg1;
- (void)outdentButtonPressed:(id)arg1;
- (void)indentButtonPressed:(id)arg1;
- (id)touchBar:(id)arg1 makeItemForIdentifier:(id)arg2;
- (id)touchBar;
- (id)textViewProvidedItemIdentifiers;
@property(readonly, nonatomic) NSWindowController *windowController;
@property(readonly, nonatomic) BOOL isRTL;
@property(readonly, nonatomic) BOOL isInMainWindow;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;
- (id)initWithTextView:(id)arg1;
- (void)dealloc;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

