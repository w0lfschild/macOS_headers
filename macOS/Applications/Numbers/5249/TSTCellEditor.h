//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "TSDEditor-Protocol.h"
#import "TSKChangeSourceObserver-Protocol.h"
#import "TSTDateTimeEditorHelperDelegate-Protocol.h"
#import "TSTFormulaReferenceHighlightPermitting-Protocol.h"

@class NSSet, NSString, TSDEditorController, TSDInteractiveCanvasController, TSKDocumentRoot, TSTCellSelection, TSTDateTimeEditorHelper, TSTInfo, TSTTableEditor;

@interface TSTCellEditor : NSObject <TSTDateTimeEditorHelperDelegate, TSDEditor, TSKChangeSourceObserver, TSTFormulaReferenceHighlightPermitting>
{
    TSKDocumentRoot *_documentRoot;
    TSDEditorController *_editorController;
    BOOL _movingBetweenNonFormulaCells;
    TSTDateTimeEditorHelper *_dateTimeEditorHelper;
}

+ (id)textEditorForSelectionPath:(id)arg1 atCellID:(struct TSUCellCoord)arg2 withNewEditors:(id)arg3;
+ (BOOL)shouldDiscardTextSelectionInSelectionPath:(id)arg1 withOldSelectionPath:(id)arg2;
+ (id)editorFromEditorController:(id)arg1;
+ (Class)stockCellEditorClass;
+ (Class)autofillEditorClass;
+ (Class)controlCellEditorClass;
@property(retain, nonatomic) TSTDateTimeEditorHelper *dateTimeEditorHelper; // @synthesize dateTimeEditorHelper=_dateTimeEditorHelper;
@property(nonatomic) BOOL movingBetweenNonFormulaCells; // @synthesize movingBetweenNonFormulaCells=_movingBetweenNonFormulaCells;
@property(retain, nonatomic) TSDEditorController *editorController; // @synthesize editorController=_editorController;
- (void).cxx_destruct;
- (id)_selectionBehaviorToRestartEditingOnUndoRedo;
- (void)_restartEditingToCommitContentWithBlock:(CDUnknownBlockType)arg1;
- (void)_restartEditingToCommitContentWithCommandBlock:(CDUnknownBlockType)arg1;
- (void)adjustCellDurationValueUnit:(int)arg1 byValue:(long long)arg2;
- (BOOL)_canToggleTimePeriodInCell:(id)arg1;
- (void)toggleTimePeriod:(id)arg1;
- (void)insertCurrentTime:(id)arg1;
- (void)insertCurrentDate:(id)arg1;
- (void)adjustCellDateValueCalendarUnit:(unsigned long long)arg1 byValue:(long long)arg2;
- (int)canPerformEditorAction:(SEL)arg1 withSender:(id)arg2;
- (void)selectionDidChangeFromSelection:(id)arg1 toSelection:(id)arg2 withFlags:(unsigned long long)arg3;
- (id)selectionWillChangeFromSelection:(id)arg1 toSelection:(id)arg2 withFlags:(unsigned long long)arg3 inSelectionPath:(id)arg4 withNewEditors:(id)arg5;
- (id)nextEditorForSelection:(id)arg1 withNewEditorStack:(id)arg2 selectionPath:(id)arg3;
- (void)willResignCurrentEditor;
- (void)didBecomeCurrentEditorForEditorController:(id)arg1;
- (BOOL)shouldDiscardAllSubselectionsStartingWithSelection:(id)arg1 withNewEditorStack:(id)arg2 selectionPath:(id)arg3;
- (BOOL)shouldRemainOnEditorStackForSelection:(id)arg1 inSelectionPath:(id)arg2 withNewEditors:(id)arg3;
@property(readonly, nonatomic) TSKDocumentRoot *documentRoot;
@property(readonly, nonatomic) BOOL permitsFormulaReferenceHighlights;
- (id)displayedObjectsForInspector;
@property(readonly, nonatomic) TSTInfo *tableInfo;
@property(readonly, nonatomic) TSDInteractiveCanvasController *interactiveCanvasController;
- (id)cellTextEditor;
@property(readonly, nonatomic) TSTTableEditor *tableEditor;
@property(readonly, nonatomic) TSTCellSelection *selection;
- (id)initWithDocumentRoot:(id)arg1 editorController:(id)arg2;
- (BOOL)numbersReorganizeInspectorPanesShouldContinueEditorStackTraversal;
- (id)numbersReorganizeInspectorPaneIdentifiers;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingCalloutAttributes;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingConnectionLineAttributes;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingEndpoints;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingFill;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingOpacity;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingReflection;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingResize;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingShadow;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingShrinkTextToFit;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingSmartShapeAttributes;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingStroke;
@property(readonly, nonatomic) NSSet *selectedObjectsSupportingTextInset;
@property(readonly) Class superclass;

@end

