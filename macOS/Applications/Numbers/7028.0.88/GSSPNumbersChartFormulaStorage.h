//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <TSCoreSOS/GSSPMessageBase.h>

#import "GSSPAutoDecodable-Protocol.h"

@class NSArray;

@interface GSSPNumbersChartFormulaStorage : GSSPMessageBase <GSSPAutoDecodable>
{
    NSArray *mDataFormulae;
    NSArray *mRowLabelFormulae;
    NSArray *mColLabelFormulae;
    NSArray *mErrorCustomPosFormulae;
    NSArray *mErrorCustomNegFormulae;
    NSArray *mErrorCustomPosScatterXFormulae;
    NSArray *mErrorCustomNegScatterXFormulae;
    int mDirection;
    BOOL mDefinedDirection;
}

+ (Class)realClass;
+ (void)registerRealGsspClass:(Class)arg1;
+ (CDUnknownFunctionPointerType)defaultDecodeConversionFunc;
+ (CDUnknownFunctionPointerType)autoDecodeConversionFunc;
+ (void)setAutoDecodeConversionFuncForDocServer:(CDUnknownFunctionPointerType)arg1;
+ (Class)autoDecodedClass;
- (void).cxx_destruct;
@property(readonly, nonatomic) BOOL definedDirection; // @synthesize definedDirection=mDefinedDirection;
- (id)description;
- (id)dspClassName;
- (id)jsonNSDictionaryValue;
- (void)populateJsonNSDictionaryValueToDictionary:(id)arg1;
- (int)p_getDiffEncodingScoreWith:(id)arg1;
- (void)p_populateStructHashKey:(struct GSSPHashKeyBuilder *)arg1;
- (unsigned long long)p_getStructHashKeyLength;
- (int)getStructHashId;
- (void)serializeRequiredFieldsWithSerializationContext:(struct GSSPSerializationContext *)arg1 withNextFieldNeedSerialize:(BOOL)arg2 diffBy:(id)arg3;
- (void)serializeOptionalFieldsWithSerializationContext:(struct GSSPSerializationContext *)arg1 withNextFieldNeedSerialize:(BOOL)arg2 diffBy:(id)arg3;
- (id)dspTypeIdString;
- (void)populateRequiredField:(int)arg1 fromContext:(struct GSSPDeserializationContext *)arg2;
- (void)populateOptionalField:(int)arg1 fromContext:(struct GSSPDeserializationContext *)arg2;
- (void)shallowCopyAllFieldsFrom:(id)arg1;
- (BOOL)clearDirection;
- (void)setDirection:(int)arg1;
- (int)direction;
- (void)setErrorCustomNegScatterXFormulae:(id)arg1;
- (id)errorCustomNegScatterXFormulae;
- (void)setErrorCustomPosScatterXFormulae:(id)arg1;
- (id)errorCustomPosScatterXFormulae;
- (void)setErrorCustomNegFormulae:(id)arg1;
- (id)errorCustomNegFormulae;
- (void)setErrorCustomPosFormulae:(id)arg1;
- (id)errorCustomPosFormulae;
- (void)setColLabelFormulae:(id)arg1;
- (id)colLabelFormulae;
- (void)setRowLabelFormulae:(id)arg1;
- (id)rowLabelFormulae;
- (void)setDataFormulae:(id)arg1;
- (id)dataFormulae;
- (id)gsspFormulasOfType:(unsigned long long)arg1;
- (void)setFormulas:(id)arg1 ofType:(unsigned long long)arg2;

@end

