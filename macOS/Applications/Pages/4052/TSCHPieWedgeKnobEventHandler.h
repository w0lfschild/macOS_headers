//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "TSDMouseEventHandler-Protocol.h"

@class NSEvent, NSString, TSCHChartPieWedgeKnob, TSCHChartRep;
@protocol TSDMouseEventHandler;

@interface TSCHPieWedgeKnobEventHandler : NSObject <TSDMouseEventHandler>
{
    TSCHChartRep *mRep;
    NSEvent *mInitEvent;
    TSCHChartPieWedgeKnob *mInitKnob;
    struct CGPoint mPointForMouseDown;
    NSEvent *mEventForMouseDown;
    NSObject<TSDMouseEventHandler> *mUnderlyingHandler;
}

- (BOOL)respondsToSelector:(SEL)arg1;
- (id)forwardingTargetForSelector:(SEL)arg1;
- (void)mouseUpAtPoint:(struct CGPoint)arg1 withEvent:(id)arg2;
- (void)mouseDraggedAtPoint:(struct CGPoint)arg1 withEvent:(id)arg2;
- (void)mouseDownAtPoint:(struct CGPoint)arg1 withEvent:(id)arg2;
- (unsigned long long)p_pieSeriesHitAtPoint:(struct CGPoint)arg1;
- (id)p_currentlySelectedSeries;
- (id)p_chartEditor;
- (id)chartRep;
- (void)dealloc;
- (id)initWithRep:(id)arg1 knob:(id)arg2 event:(id)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

