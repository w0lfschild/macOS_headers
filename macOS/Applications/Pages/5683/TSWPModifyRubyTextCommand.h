//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "TSWPTextBaseCommand.h"

@class NSString, TSWPChangeSession, TSWPRubyField, TSWPStorage;

@interface TSWPModifyRubyTextCommand : TSWPTextBaseCommand
{
    TSWPStorage *_storage;
    TSWPRubyField *_rubyField;
    NSString *_rubyText;
    NSString *_baseText;
    unsigned long long _uiSessionID;
    TSWPChangeSession *_changeSession;
    NSString *_language;
    struct TSWPStorageTransaction *_undoRedoTransaction;
}

+ (struct _NSRange)p_modifyRubyField:(id)arg1 baseText:(id)arg2 changeSession:(id)arg3 language:(id)arg4 undoTransaction:(struct TSWPStorageTransaction *)arg5;
@property(nonatomic) struct TSWPStorageTransaction *undoRedoTransaction; // @synthesize undoRedoTransaction=_undoRedoTransaction;
@property(copy, nonatomic) NSString *language; // @synthesize language=_language;
@property(nonatomic) __weak TSWPChangeSession *changeSession; // @synthesize changeSession=_changeSession;
@property(nonatomic) unsigned long long uiSessionID; // @synthesize uiSessionID=_uiSessionID;
@property(copy, nonatomic) NSString *baseText; // @synthesize baseText=_baseText;
@property(copy, nonatomic) NSString *rubyText; // @synthesize rubyText=_rubyText;
@property(retain, nonatomic) TSWPRubyField *rubyField; // @synthesize rubyField=_rubyField;
- (void).cxx_destruct;
- (void)coalesceWithCommand:(id)arg1 coalescingKind:(long long)arg2;
- (BOOL)canCoalesceWithCommand:(id)arg1 coalescingKind:(long long)arg2;
- (void)redo;
- (void)undo;
- (void)p_undoRedo;
- (void)commit;
- (BOOL)process;
- (void)p_swapRubyTexts;
- (id)actionString;
- (void)loadFromUnarchiver:(id)arg1;
- (void)loadFromArchive:(const struct ModifyRubyTextCommandArchive *)arg1 unarchiver:(id)arg2;
- (void)saveToArchiver:(id)arg1;
- (void)saveToArchive:(struct ModifyRubyTextCommandArchive *)arg1 archiver:(id)arg2;
- (void)dealloc;
@property(readonly, nonatomic) __weak TSWPStorage *storage;
- (id)initWithRubyField:(id)arg1 baseText:(id)arg2 rubyText:(id)arg3 rubyUISession:(unsigned long long)arg4 language:(id)arg5;
- (id)initWithRubyField:(id)arg1 baseText:(id)arg2 rubyText:(id)arg3 rubyUISession:(unsigned long long)arg4 changeSession:(id)arg5 language:(id)arg6;
- (struct TSWPStorageTransaction *)forgetUndoRedoTransaction;

@end

