//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSMutableString.h>

@interface NSMutableString (WETStringAdditions)
- (void)tc_replaceTimeFormatString:(id)arg1 with:(id)arg2;
- (void)gq_insertCharacter:(unsigned short)arg1 atIndex:(unsigned short)arg2;
- (void)ice_pAppendRange:(struct _NSRange)arg1;
- (void)tc_replaceAllSubstr:(id)arg1 with:(id)arg2 escStr:(id)arg3 range:(struct _NSRange)arg4;
- (void)tc_replaceAllSubstr:(id)arg1 with:(id)arg2 range:(struct _NSRange)arg3;
- (void)tc_replaceAllSubstr:(id)arg1 with:(id)arg2 escStr:(id)arg3;
- (void)tc_replaceAllSubstr:(id)arg1 with:(id)arg2;
- (BOOL)tc_replaceSubstr:(id)arg1 with:(id)arg2 range:(struct _NSRange)arg3;
- (BOOL)tc_replaceSubstr:(id)arg1 with:(id)arg2;
@end

