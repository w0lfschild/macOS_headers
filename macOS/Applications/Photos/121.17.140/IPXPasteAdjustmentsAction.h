//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "IPXEditAction.h"

@class IPAPhotoAdjustmentStack, NSMapTable;

__attribute__((visibility("hidden")))
@interface IPXPasteAdjustmentsAction : IPXEditAction
{
    IPAPhotoAdjustmentStack *_copiedAdjustmentStack;
    NSMapTable *_savedDescriptions;
}

+ (id)sharedAdjustmentPasteboardController;
+ (void)clearAdjustmentsPasteboard;
+ (void)pasteToPrimarySelection:(id)arg1 externalEditSessionController:(id)arg2 window:(id)arg3 undoManager:(id)arg4;
+ (void)copyFromImageItemController:(id)arg1;
+ (id)_stripDisabledOperationsFromDescription:(id)arg1;
+ (void)copyFromPrimarySelection:(id)arg1 viewerController:(id)arg2;
+ (BOOL)canPasteToAsset:(id)arg1 viewerItemController:(id)arg2;
+ (BOOL)canPasteToAsset:(id)arg1 viewerController:(id)arg2;
+ (BOOL)canCopyFromAsset:(id)arg1 imageItemController:(id)arg2;
+ (BOOL)canCopyFromAsset:(id)arg1 viewerController:(id)arg2;
- (void).cxx_destruct;
- (void)performActionWithImageItemController:(id)arg1;
- (id)_filterAdjustmentStack:(id)arg1 padec:(id)arg2;
- (id)defaultActionName;
- (id)initWithAdjustmentStack:(id)arg1 asset:(id)arg2;

@end

