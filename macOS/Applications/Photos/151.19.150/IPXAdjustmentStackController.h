//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "IPXScrollableEditToolContentController.h"

#import "IPXAdjustmentBrickControllerDelegate-Protocol.h"
#import "IPXHistogramControllerDelegate-Protocol.h"
#import "IPXTouchBarProvider-Protocol.h"
#import "PAImageAssetClientDelegate-Protocol.h"

@class IPXAdjustmentTouchBarController, IPXEditToolHeaderView, IPXEditUndoRedo, IPXHistogramController, IPXLevelsHistogramView, IPXViewerOverlayController, NSArray, NSButton, NSLayoutConstraint, NSMutableArray, NSMutableOrderedSet, NSStackView, NSString, NSTrackingArea, NSView, PFCanceler, PFTimeIntervalCoalescer, UXView;
@protocol PXDisplayAsset;

__attribute__((visibility("hidden")))
@interface IPXAdjustmentStackController : IPXScrollableEditToolContentController <PAImageAssetClientDelegate, IPXAdjustmentBrickControllerDelegate, IPXHistogramControllerDelegate, IPXTouchBarProvider>
{
    NSStackView *_stackView;
    NSMutableArray *_brickControllers;
    NSMutableOrderedSet *_versionDefaultIdentifiers;
    NSString *_currentItemControllerAssetUUID;
    PFCanceler *_megaThumbCanceler;
    PFTimeIntervalCoalescer *_megaThumbCoalescer;
    BOOL _hasChangesForVersion;
    BOOL _defaultOperationsReady;
    BOOL _shuttingDown;
    IPXEditToolHeaderView *_headerView;
    NSArray *_currentVersionThumbs;
    UXView *_histogramContainer;
    IPXLevelsHistogramView *_histogramView;
    NSView *_histogramSeparatorView;
    IPXHistogramController *_histogramController;
    NSLayoutConstraint *_collapseHistogramConstraint;
    NSButton *_histogramPopoutButton;
    NSButton *_histogramExpandButton;
    NSTrackingArea *_trackingArea;
    NSArray *_histogramNotVisibleConstraints;
    NSArray *_histogramVisibleConstraints;
    IPXEditUndoRedo *_currentInteractiveAction;
    long long _currentInputSource;
    IPXAdjustmentTouchBarController *_touchBarController;
    long long _histogramVisibleState;
    IPXViewerOverlayController *_overlayController;
}

+ (void)_setDefaultHistogramColorSpace:(id)arg1;
+ (id)_defaultHistogramColorSpace;
+ (double)adjustmentAnimationDuration;
+ (void)initialize;
- (void).cxx_destruct;
- (id)overlayController;
- (id)touchBar:(id)arg1 makeItemForIdentifier:(id)arg2;
- (id)touchBarIdentifiers;
- (void)synchronizeModifierFlags:(unsigned long long)arg1;
- (void)histogramEndTracking:(id)arg1;
- (void)histogram:(id)arg1 parameterChangedValue:(id)arg2 forChannel:(long long)arg3;
- (void)histogramBeginTracking:(id)arg1;
- (void)a_selectHistogramColorSpace:(id)arg1;
- (void)setHistogramActiveChannel:(long long)arg1;
- (void)a_selectNewHistogramChannel:(id)arg1;
- (BOOL)validateMenuItem:(id)arg1;
- (void)addHistogramPopOutMenuItemsToMenu:(id)arg1;
- (void)a_showHistogramPopoutMenu:(id)arg1;
- (void)_updateHistogram:(id)arg1;
- (void)_setHistogramColorSpace:(id)arg1;
- (id)_histogramColorSpaceName;
- (void)histogramController:(id)arg1 didRenderHistogram:(id)arg2;
- (void)_initializeHistogramController;
- (void)a_toggleHistogramExpansion:(id)arg1;
- (id)histogramExpansionButtonView;
- (void)_setHistogramVisible:(long long)arg1 animated:(BOOL)arg2;
- (void)prepareForRevertToOriginal;
- (BOOL)_expandedStateForIdentifier:(id)arg1;
- (void)_recycleBricks;
- (void)_saveDefaultIdentifiers:(id)arg1;
- (void)_saveDefaultIdentifiers;
- (id)_storedDefaultIdentifiers;
- (unsigned long long)_orderedIndexForIdentifier:(id)arg1;
- (id)_imageItemController;
- (void)_ensureOperationExistsForBrickController:(id)arg1;
- (void)_ensureOperationExistsForBrickController:(id)arg1 activateIfCreating:(BOOL)arg2;
- (BOOL)_operationExistsForBrickController:(id)arg1;
- (id)brickControllerForIdentifier:(id)arg1;
- (id)_brickControllersByIdentifier;
- (void)_removeBrickController:(id)arg1 removeOperation:(BOOL)arg2 completion:(CDUnknownBlockType)arg3;
- (void)_defaultOperationsReady;
- (void)_reloadMegaSliders;
- (void)_reloadMegaSlidersAfterDelay;
- (void)_handleLoadThumbnailsResponse:(id)arg1 forBrickController:(id)arg2 asset:(id)arg3;
- (void)_loadThumbnailsForBrickController:(id)arg1;
- (id)_defaultOperationIdentifiers;
- (id)_allUIIdentifiers;
- (id)_allIdentifiers;
- (id)_excludedOperations;
- (void)_loadBricksWithThumbs:(BOOL)arg1 canReprocessVersion:(BOOL)arg2;
- (void)_setupBrickControllers;
- (void)_removeOperationForIdentifier:(id)arg1 descriptionEditController:(id)arg2;
- (BOOL)_canRevertAdjustmentsForAsset:(id)arg1;
- (void)updateResetButton;
- (BOOL)resetEnabled;
- (id)resetTarget;
- (id)resetTooltip;
- (id)resetLabel;
- (SEL)resetSelector;
- (void)a_resetAdjustments:(id)arg1;
- (void)a_saveAsDefaults:(id)arg1;
- (void)assetDidBecomeAvailable;
- (void)assetDidChangeOrientation;
- (void)assetDidChangeOperationAtIndex:(long long)arg1;
- (void)assetDidChangeOperations;
- (id)assetForBrickController:(id)arg1;
- (void)brickController:(id)arg1 didHideOverlay:(id)arg2;
- (void)brickController:(id)arg1 didShowOverlay:(id)arg2;
- (BOOL)brickControllerExpandedState:(id)arg1;
- (void)brickControllerExpansionAnimationCompleted:(id)arg1;
- (void)brickControllerDidExpand:(id)arg1;
- (void)brickControllerDidEndInteractiveChange:(id)arg1;
- (void)brickControllerDidBeginInteractiveChange:(id)arg1 forKey:(id)arg2 inputSource:(long long)arg3;
- (void)brickControllerEnsureOperationExists:(id)arg1;
- (BOOL)isAdjustmentControllerInInteractiveSession;
- (void)brickController:(id)arg1 didChangeValue:(id)arg2 forKey:(id)arg3;
- (void)brickControllerDidSetAlternateAuto:(id)arg1;
- (void)brickControllerDidSetAuto:(id)arg1;
- (void)handleCompletedAutoCalculator:(id)arg1 operationUUID:(id)arg2 savedRenderDescription:(id)arg3 savedSnapshot:(id)arg4;
- (void)brickController:(id)arg1 didChangeActiveState:(BOOL)arg2;
- (void)brickControllerDidToggleMembershipInDefaultSet:(id)arg1;
- (BOOL)brickControllerIsInDefaultSet:(id)arg1;
- (void)brickControllerDidReset:(id)arg1;
- (void)brickControllerDidRevert:(id)arg1;
- (void)brickControllerDidRemove:(id)arg1;
- (id)imageItemController;
- (void)_registerBlockForUndo:(CDUnknownBlockType)arg1 adding:(BOOL)arg2 identifiers:(id)arg3;
- (void)_addIdentifierToVersionDefaultIdentifiers:(id)arg1;
- (void)_removeIdentifierFromVersionDefaultIdentifiers:(id)arg1;
- (void)_addBricksForAdjustmentIdentifiers:(id)arg1 animated:(BOOL)arg2;
- (void)_animateBrickIn:(id)arg1 startDelay:(double)arg2 initial:(BOOL)arg3 expanded:(BOOL)arg4;
- (id)_addBrickForAdjustmentIdentifier:(id)arg1 descriptionEditController:(id)arg2;
- (void)willDeselectItemController:(id)arg1;
- (void)_tryToLoadBricksForDidSelectItemController;
- (void)_handleMegasliderThumbnailRequestResult:(id)arg1;
- (void)didSelectItemController:(id)arg1;
- (void)willSelectItemController:(id)arg1;
- (void)mouseExited:(id)arg1;
- (void)mouseEntered:(id)arg1;
- (void)setHoverControlsHidden:(BOOL)arg1;
- (void)_reloadBricks;
- (void)_addConstraintsForHeaderAndScroll;
- (CDUnknownBlockType)animationOutCompletion;
- (CDUnknownBlockType)animationOut;
- (CDUnknownBlockType)preAnimationOut;
- (CDUnknownBlockType)animationInCompletion;
- (CDUnknownBlockType)animationIn;
@property(readonly, nonatomic) id <PXDisplayAsset> _asset;
- (void)_updateStackViewEdgeInsetsForHistogramVisible:(long long)arg1 hasAlertAsFirstBrick:(BOOL)arg2 animate:(BOOL)arg3;
- (void)_updateStackViewEdgeInsetsAnimated:(BOOL)arg1;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void *)arg4;
- (void)viewWillDisappear;
- (void)viewWillAppear;
- (void)loadView;
- (void)willExit:(id)arg1 completion:(CDUnknownBlockType)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly, nonatomic) NSArray *secondaryTouchBarIdentifiers;
@property(readonly) Class superclass;

@end

