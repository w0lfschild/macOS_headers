//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "IPXObjectsController.h"

#import "IPXObjectsControllerChangeReceiver-Protocol.h"

@class IPXRecentActivityAssetsController, IPXSharedStreamsController, NSArray, NSString, RDDatabase;
@protocol IPXCollection;

@interface IPXSharedObjectsController : IPXObjectsController <IPXObjectsControllerChangeReceiver>
{
    IPXRecentActivityAssetsController *_recentActivityAssetsController;
    IPXSharedStreamsController *_sharedStreamsController;
    id <IPXCollection> _currentSharedStreamsCollection;
    RDDatabase *_database;
}

+ (id)defaultUUID;
@property(retain) RDDatabase *database; // @synthesize database=_database;
@property(retain) id <IPXCollection> currentSharedStreamsCollection; // @synthesize currentSharedStreamsCollection=_currentSharedStreamsCollection;
@property(retain) IPXSharedStreamsController *sharedStreamsController; // @synthesize sharedStreamsController=_sharedStreamsController;
@property(retain) IPXRecentActivityAssetsController *recentActivityAssetsController; // @synthesize recentActivityAssetsController=_recentActivityAssetsController;
- (void).cxx_destruct;
- (void)objectsController:(id)arg1 representativeImageProperties:(unsigned long long)arg2 changedForObjects:(id)arg3;
- (void)objectsController:(id)arg1 metadataChanged:(id)arg2 forObjects:(id)arg3;
- (void)objectsController:(id)arg1 propertiesChanged:(id)arg2;
- (void)objectsControllerObjectsCollectionChanged:(id)arg1;
- (void)updateContent:(CDUnknownBlockType)arg1;
- (id)sharedStreamForAlbumModelId:(unsigned long long)arg1;
@property(readonly) NSArray *subscribers;
@property(readonly) unsigned long long sharedStreamCount;
@property(readonly) NSArray *editableSharedStreams;
@property(readonly) NSArray *sharedStreams;
@property(readonly) BOOL isAnySharedStreamAvailable;
- (void)uninstallObjectsSourceMonitor;
- (void)installObjectsSourceMonitor;
- (void)stopMonitoringObjectsSource;
- (void)startMonitoringObjectsSourceAndWaitForInitialData:(double)arg1 completion:(CDUnknownBlockType)arg2;
- (id)objectsCollectionName;
- (void)dealloc;
- (id)initWithDatabase:(id)arg1;
- (void)_updateCurrentContentObjects:(CDUnknownBlockType)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

