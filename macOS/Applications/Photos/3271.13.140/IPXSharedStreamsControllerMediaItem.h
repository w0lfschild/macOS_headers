//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <Mondrian/MOMediaItem.h>

@class IPXSharedStreamsController;

@interface IPXSharedStreamsControllerMediaItem : MOMediaItem
{
    IPXSharedStreamsController *_sharedStreamsController;
}

@property(readonly) IPXSharedStreamsController *sharedStreamsController; // @synthesize sharedStreamsController=_sharedStreamsController;
- (void).cxx_destruct;
- (id)thumbnailImageOperationForResolution:(unsigned long long)arg1 size:(struct CGSize)arg2 mode:(unsigned long long)arg3 reason:(unsigned long long)arg4 placeholder:(BOOL)arg5 colorSpace:(id)arg6 withCompletionHandler:(CDUnknownBlockType)arg7;
- (void)_reloadProperties;
- (void)updateWithSharedStreamsController:(id)arg1;
- (id)initWithSharedStreamsController:(id)arg1;

@end

