//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "IPXViewerItemController.h"

#import "NSMenuItemValidation-Protocol.h"
#import "PADescriptionEditReceiver-Protocol.h"
#import "PAImageActivityReceiver-Protocol.h"
#import "PAImageItemViewControllerStatusReceiver-Protocol.h"
#import "PFActionNotificationsReceiver-Protocol.h"

@class IPXDelayedTimer, NSObject, NSProgressIndicator, NSString, NSUndoManager, NSView, PADescriptionEditController, PAImageItemController, PAImageItemView, PAImageItemViewController, PAImageItemViewMode, _IPXViewerImageEditReceiver;
@protocol OS_dispatch_queue, PXDisplayAsset;

__attribute__((visibility("hidden")))
@interface IPXViewerImageController : IPXViewerItemController <PFActionNotificationsReceiver, PAImageActivityReceiver, NSMenuItemValidation, PAImageItemViewControllerStatusReceiver, PADescriptionEditReceiver>
{
    NSView *_loadingIndicatorView;
    NSProgressIndicator *_spinner;
    BOOL _finishedLoading;
    BOOL _shouldShowLoadingIndicatorAfterDelay;
    IPXDelayedTimer *_straightenTimer;
    NSObject *_observedCanvasView;
    PADescriptionEditController *_descriptionEditController;
    _IPXViewerImageEditReceiver *_descriptionEditReceiver;
    NSObject<OS_dispatch_queue> *_editingQueue;
    double _zoomFactor;
    long long _originalRenderMode;
    PAImageItemViewMode *_normalMode;
    PAImageItemViewMode *_originalMode;
    BOOL _isObservingNotifications;
    long long _rotationAnimatingCount;
    BOOL _preservedUserInteractionEnabled;
    id <PXDisplayAsset> _testElement_timeToSharpAsset;
    BOOL _testElement_isAssetSharp;
    CDUnknownBlockType _testElement_timeToSharpCompletionBlock;
    BOOL _showsBurstBadge;
    BOOL _showsBurstFavoritingCheckmark;
    BOOL _isAnimatingOrientationChange;
    PAImageItemController *_imageItemController;
    NSProgressIndicator *_loadingIndicator;
    long long _adjustmentLoadingState;
}

- (void).cxx_destruct;
@property(readonly, nonatomic) BOOL isAnimatingOrientationChange; // @synthesize isAnimatingOrientationChange=_isAnimatingOrientationChange;
@property(nonatomic) long long adjustmentLoadingState; // @synthesize adjustmentLoadingState=_adjustmentLoadingState;
@property(readonly, nonatomic) NSProgressIndicator *loadingIndicator; // @synthesize loadingIndicator=_loadingIndicator;
@property(nonatomic) BOOL showsBurstFavoritingCheckmark; // @synthesize showsBurstFavoritingCheckmark=_showsBurstFavoritingCheckmark;
@property(nonatomic) BOOL showsBurstBadge; // @synthesize showsBurstBadge=_showsBurstBadge;
@property(readonly, nonatomic) PAImageItemController *imageItemController; // @synthesize imageItemController=_imageItemController;
- (void)imageActivityControllerDidLoadImage:(id)arg1 quality:(unsigned long long)arg2 finalQuality:(unsigned long long)arg3;
- (void)_testElement_handleCanvasOrPreviewReadyWithCompletion:(CDUnknownBlockType)arg1;
- (void)testElement_waitForAssetToBecomeSharp:(CDUnknownBlockType)arg1;
- (void)testElement_stopListeningToTimeToSharp;
- (void)testElement_startListeningToTimeToSharp;
- (void)_restoreNormalMode:(id)arg1;
- (void)resetShowOriginalMode;
- (long long)originalMode;
- (void)_didLoadOriginalMode:(id)arg1;
- (BOOL)_loadMode:(id)arg1 waitFor:(double)arg2 completion:(CDUnknownBlockType)arg3;
- (void)_pushOriginalMode:(id)arg1;
- (void)setShowOriginalMode:(long long)arg1;
- (BOOL)allowsZooming;
- (BOOL)allowsEditing;
- (void)descriptionEditController:(id)arg1 renderDescriptionOrientationChanged:(id)arg2;
- (void)_animateOverlayFadeForOrientationChange:(id)arg1;
- (void)descriptionEditController:(id)arg1 videoComplementVisibilityChanged:(BOOL)arg2;
- (void)descriptionEditController:(id)arg1 renderDescription:(id)arg2 endInteractiveForOperationAtIndex:(unsigned long long)arg3;
- (void)descriptionEditController:(id)arg1 renderDescription:(id)arg2 beginInteractiveForOperationAtIndex:(unsigned long long)arg3;
- (void)descriptionEditController:(id)arg1 renderDescription:(id)arg2 operationDidChangeAtIndex:(unsigned long long)arg3 invalidMasterRect:(struct CGRect)arg4;
- (void)operationsChangedForDescriptionController:(id)arg1 renderDescription:(id)arg2;
- (void)_updateLoadingIndicatorVisiblility:(BOOL)arg1;
- (void)viewController:(id)arg1 statusChanged:(int)arg2 estimatedTimeUntilUpToDate:(double)arg3;
- (id)multicaster:(id)arg1 queueForSelector:(SEL)arg2;
- (void)updateShowsBurstBadge;
- (BOOL)_shouldShowBurstBadge;
- (void)_animateOverlaysForOrientationChange;
- (void)_applyOrientationChange:(long long)arg1;
- (BOOL)isAnimatingRotation;
- (void)_animateApplyOrientationChange:(long long)arg1 completion:(CDUnknownBlockType)arg2;
- (void)action:(id)arg1 statusChangedFrom:(int)arg2 to:(int)arg3;
- (void)flipVertical;
- (void)flipHorizontal;
- (void)rotateLeft;
- (void)rotateRight;
- (void)_rotateBy:(long long)arg1;
- (BOOL)_prepareOverlaysForRotate;
- (void)_flipVertically:(BOOL)arg1;
- (void)_prepareOverlaysForFlip;
- (void)a_reprocessRaw:(id)arg1;
- (BOOL)validateMenuItem:(id)arg1;
- (void)_imageItemViewDidChangeDisplay:(id)arg1;
- (struct CGPoint)panOffsetForShowingContentPoint:(struct CGPoint)arg1 atViewPoint:(struct CGPoint)arg2;
- (struct CGPoint)contentPointAtViewPoint:(struct CGPoint)arg1;
- (struct CGPoint)panOffsetForViewPoint:(struct CGPoint)arg1;
- (BOOL)acceptsFirstResponder;
- (id)_imageItemViewModeForViewerMode:(long long)arg1;
@property(readonly, nonatomic) PAImageItemViewMode *imageItemViewMode;
- (void)_updateZoomOverlay;
- (BOOL)_zoomOverlayShouldBeActive;
- (BOOL)isZoomed;
- (void)setZoomValue:(double)arg1;
- (double)zoomFactor;
- (void)_updatePanOffset;
- (void)_updateZoomFactor;
- (id)imageItemView;
- (id)itemView;
- (void)_removeItemViewController;
- (void)_stopObservingPopoverWindow;
- (void)_setupItemViewController;
@property(readonly, nonatomic) PAImageItemView *imageView;
@property(readonly, nonatomic) PAImageItemViewController *imagingController;
- (id)_newPAImageItemViewController;
- (BOOL)prepareLayerForAnimationContainingCurrentCanvasViewContentsWithCompletionHandler:(CDUnknownBlockType)arg1;
- (id)canvasContentLayer;
- (struct CGImage *)_currentCanvasViewContents;
- (id)_layerForOneUpTransitionContainingCurrentCanvasViewContents;
- (id)_layerForAnimationContainingCurrentCanvasViewContents;
- (id)_layerForPreviewContents;
@property(readonly, nonatomic) NSUndoManager *initalUndoManager;
- (void)didFinishAnimatingOrientationChange;
- (void)willAnimateOrientationChange;
- (void)updateProgressIndicatorDisplay;
@property(readonly, nonatomic) BOOL needsProgressIndicator;
- (long long)requestHighQualityFormatDownloadIfNeeded;
- (void)assetLoadingStateDidChange;
- (void)assetDidChange:(id)arg1;
- (void)setRenderMode:(long long)arg1 completion:(CDUnknownBlockType)arg2;
- (double)zoomValueForZoomingBy:(double)arg1;
- (void)setViewFrame:(struct CGRect)arg1 animated:(BOOL)arg2;
- (void)didEndLiveResize;
- (void)willStartLiveResize;
- (void)setEditing:(BOOL)arg1 saveTrimAsNewVideo:(BOOL)arg2 completion:(CDUnknownBlockType)arg3;
- (void)_finishCommitEditing:(BOOL)arg1;
- (void)didFailToEnterEditWithError:(id)arg1;
- (void)_finishBeginEditing:(BOOL)arg1 error:(id)arg2 completion:(CDUnknownBlockType)arg3;
- (void)loadAdjustmentFactory:(CDUnknownBlockType)arg1;
- (id)adjustmentFactory;
- (void)didEnsureItemControllerIsReady;
- (void)preloadItem;
- (void)loadItem;
- (void)didEndAnimationInScrollView:(id)arg1;
- (void)beginAnimationInScrollView:(id)arg1;
- (void)setPanOffset:(struct CGPoint)arg1;
- (void)removeViewerItemModel;
- (void)setupViewerItemModel;
- (void)setEdgeInsets:(struct NSEdgeInsets)arg1;
- (void)removeFromParentViewController;
- (void)viewDidLoad;
- (id)initWithOwningViewerController:(id)arg1 mediaProvider:(id)arg2 options:(unsigned long long)arg3;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly, nonatomic) PAImageItemController *renderItemController; // @dynamic renderItemController;
@property(readonly) Class superclass;

@end

