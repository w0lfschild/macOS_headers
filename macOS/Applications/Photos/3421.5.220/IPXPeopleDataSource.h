//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import "IPXMondrianViewDataSource.h"

@class NSMutableDictionary;

@interface IPXPeopleDataSource : IPXMondrianViewDataSource
{
    NSMutableDictionary *_faceTileByPersonId;
}

+ (CDUnknownBlockType)personModelUuidComparator;
- (void).cxx_destruct;
- (void)invalidateKeyFacesForPeople:(id)arg1;
- (id)changeSetForTransformationToCurrentSectionsCollection:(id)arg1 withModifiedItems:(id)arg2 pinnedItems:(id)arg3;
- (id)newMediaSectionForItemsCollection:(id)arg1;
- (CDUnknownBlockType)sectionItemsEquivalencyComparator;

@end

