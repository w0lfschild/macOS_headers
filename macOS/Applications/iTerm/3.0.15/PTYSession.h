...
@class DVR, DVRDecoder, FindContext, Interval, MovingAverage, NSArray, NSColor, NSDate, NSDictionary, NSImage, NSMutableArray, NSMutableData, NSMutableDictionary, NSMutableSet, NSNumber, NSObject, NSString, NSTimer, PTYTask, PTYTextView, SessionView, TerminalFile, TextViewWrapper, TmuxController, TmuxGateway, VT100RemoteHost, VT100Screen, VT100Terminal, iTermAutomaticProfileSwitcher, iTermColorMap, iTermMark, iTermPasteHelper, iTermPromptOnCloseReason, iTermQuickLookController, iTermThroughputEstimator;
@protocol OS_dispatch_semaphore, PTYSessionDelegate;

@interface PTYSession : NSResponder <iTermAutomaticProfileSwitcherDelegate, iTermPasteHelperDelegate, iTermSessionViewDelegate, FindViewControllerDelegate, iTermWeaklyReferenceable, PopupDelegate, PTYTaskDelegate, PTYTextViewDelegate, TmuxGatewayDelegate, VT100ScreenDelegate>
{
    BOOL _registered;
    NSString *_name;
...
+ (BOOL)_recursiveSelectMenuItem:(id)arg1 inMenu:(id)arg2;
...
@property(readonly, nonatomic) VT100RemoteHost *currentHost;
...
- (void)doAntiIdle;
- (void)setUpdateCadence:(double)arg1;
...
- (id)sessionId;
- (BOOL)shouldSetCtype;
- (id)_autoLogFilenameForTermId:(id)arg1;
