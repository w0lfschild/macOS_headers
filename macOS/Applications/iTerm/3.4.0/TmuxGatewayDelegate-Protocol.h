//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSData, NSNumber, NSString, NSWindowController<iTermWindowController>, TmuxController;

@protocol TmuxGatewayDelegate <NSObject>
- (void)tmuxWindowPaneDidPause:(int)arg1 notification:(BOOL)arg2;
- (void)tmuxActiveWindowPaneDidChangeInWindow:(int)arg1 toWindowPane:(int)arg2;
- (void)tmuxGatewayDidTimeOut;
- (BOOL)tmuxGatewayShouldForceDetach;
- (NSString *)tmuxOwningSessionGUID;
- (void)tmuxDoubleAttachForSessionGUID:(NSString *)arg1;
- (void)tmuxDidOpenInitialWindows;
- (void)tmuxCannotSendCharactersInSupplementaryPlanes:(NSString *)arg1 windowPane:(int)arg2;
- (void)tmuxInitialCommandDidFailWithError:(NSString *)arg1;
- (void)tmuxInitialCommandDidCompleteSuccessfully;
- (NSWindowController<iTermWindowController> *)tmuxGatewayWindow;
- (void)tmuxPrintLine:(NSString *)arg1;
- (void)tmuxSetSecureLogging:(BOOL)arg1;
- (long long)tmuxNumberOfLinesOfScrollbackHistory;
- (CDStruct_1ef3fb1f)tmuxClientSize;
- (void)tmuxSession:(int)arg1 renamed:(NSString *)arg2;
- (void)tmuxWindowsDidChange;
- (void)tmuxSessionsChanged;
- (void)tmuxSessionChanged:(NSString *)arg1 sessionId:(int)arg2;
- (void)tmuxReadTask:(NSData *)arg1 windowPane:(int)arg2 latency:(NSNumber *)arg3;
- (void)tmuxWriteString:(NSString *)arg1;
- (void)tmuxHostDisconnected:(NSString *)arg1;
- (void)tmuxWindowRenamedWithId:(int)arg1 to:(NSString *)arg2;
- (void)tmuxWindowClosedWithId:(int)arg1;
- (void)tmuxWindowAddedWithId:(int)arg1;
- (BOOL)tmuxUpdateLayoutForWindow:(int)arg1 layout:(NSString *)arg2 zoomed:(NSNumber *)arg3 only:(BOOL)arg4;
- (TmuxController *)tmuxController;
@end

