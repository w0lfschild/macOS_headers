//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSView.h"

#import "CALayerDelegate.h"

@class NSString, iTermRateLimitedUpdate;

@interface iTermBaseMinimapView : NSView <CALayerDelegate>
{
    BOOL _invalid;
}

+ (void)postInvalidateNotification;
- (void)performInvalidate;
- (void)performInvalidateIfNeeded;
- (void)invalidate;
- (CDStruct_9d0a8532)seriesAtIndex:(long long)arg1;
- (long long)numberOfSeries;
- (void)didDraw;
- (struct _NSRange)rangeOfVisibleLines;
- (void)drawLayer:(id)arg1 inContext:(struct CGContext *)arg2;
- (void)viewDidMoveToWindow;
- (void)setHasData:(BOOL)arg1;
@property(readonly, nonatomic) iTermRateLimitedUpdate *rateLimit;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

