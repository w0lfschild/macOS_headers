//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "NSXPCListenerDelegate-Protocol.h"

@class NSHashTable, NSString, NSXPCListener;

@interface BackgroundTaskManagementService : NSObject <NSXPCListenerDelegate>
{
    NSXPCListener *_serviceListener;
    NSHashTable *_connectedProcesses;
    NSString *_serviceName;
}

@property(readonly) NSString *serviceName; // @synthesize serviceName=_serviceName;
@property(readonly) NSHashTable *connectedProcesses; // @synthesize connectedProcesses=_connectedProcesses;
@property(retain) NSXPCListener *serviceListener; // @synthesize serviceListener=_serviceListener;
- (void).cxx_destruct;
- (BOOL)listener:(id)arg1 shouldAcceptNewConnection:(id)arg2;
- (void)resume;
- (id)initWithServiceName:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

