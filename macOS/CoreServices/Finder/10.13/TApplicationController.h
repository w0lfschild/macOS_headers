//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <AppKit/NSResponder.h>

#import "NSApplicationDelegate-Protocol.h"
#import "NSMenuDelegate-Protocol.h"
#import "TNodeObserverProtocol-Protocol.h"

@class NSMenu, NSMenuItem, NSString, TArrangeByMenuController;

@interface TApplicationController : NSResponder <NSMenuDelegate, NSApplicationDelegate, TNodeObserverProtocol>
{
    struct TNSRef<NSObject *, void> _sharedIKContext;
    struct TNSRef<NSMutableSet *, void> _sharedIKContextClients;
    struct TApplicationEventHandler *_application;
    struct shared_ptr<TUndoableActionManager> _undoableActionManager;
    long long _lastRecentFoldersMenuUpdate;
    NSMenu *_goMenu;
    NSMenu *_recentFoldersMenu;
    TArrangeByMenuController *_arrangeByMenuController;
    NSMenu *_dockMenu;
    NSMenuItem *_debugMenuItem;
    struct TNSRef<NSMenuItem *, void> _newWindowAsTabMenuItem;
    long long _userTabbingPreference;
    struct TNSRef<TMidnightTimer *, void> _timerToUpdateStuffAtMidnight;
    struct TNSRef<NSBackgroundActivityScheduler *, void> _removeOldTrashItemsActivity;
    struct TNSRef<TICloudAccountManager *, void> _iCloudAccountMonitor;
    struct shared_ptr<TNodeObserverCocoaBridge> _libraryNodeObserver;
    struct TNotificationCenterObserver _anyWindowBecameMainObserver;
    struct TNotificationCenterObserver _windowMenuDidSendActionObserver;
    struct TNotificationCenterObserver _logTaggingOpsFinderDayChangedObserver;
    struct vector<TNotificationCenterObserver, std::__1::allocator<TNotificationCenterObserver>> _observers;
    struct TFENode _handoffNode;
    _Bool _handoffShouldViewParentContainer;
    struct TNSRef<NSUserActivity *, void> _handoffActivity;
}

+ (void)updateFileMenu;
+ (id)helpMenu;
+ (id)debugMenu;
+ (id)windowMenu;
+ (id)viewMenu;
+ (id)editMenu;
+ (id)fileMenu;
+ (id)finderMenu;
+ (id)menubar;
+ (id)instanceMethodSignatureForSelector:(SEL)arg1;
+ (_Bool)shouldForwardSelectorToGlobalWindowController:(SEL)arg1;
+ (id)sharedApplicationController;
+ (void)startQuitSequence;
@property(nonatomic) long long userTabbingPreference; // @synthesize userTabbingPreference=_userTabbingPreference;
- (id).cxx_construct;
- (void).cxx_destruct;
- (void)revealHandoffItemWithURL:(id)arg1 viewingContainer:(_Bool)arg2;
- (BOOL)application:(id)arg1 continueUserActivity:(id)arg2 restorationHandler:(CDUnknownBlockType)arg3;
- (void)unregisterHandoffActivity;
- (void)registerHandoffActivityForSelection:(const struct TFENodeVector *)arg1 target:(const struct TFENode *)arg2;
- (id)applicationDockMenu:(id)arg1;
- (void)applicationWillTerminate:(id)arg1;
- (unsigned long long)applicationShouldTerminate:(id)arg1;
- (void)applicationDidBecomeActive:(id)arg1;
- (void)applicationDidFinishLaunching:(id)arg1;
- (void)applicationWillFinishLaunching:(id)arg1;
- (void)checkSingleInstance;
- (void)terminateSelf;
- (_Bool)terminateOtherFinder;
- (void)cmdBrowseOtherTimeMachineDisks:(id)arg1;
- (void)notifyWithFrontPaths;
- (void)cmdEnterTimeMachine:(id)arg1;
- (void)cmdCycleWindows:(id)arg1;
- (_Bool)validateCycleWindows:(id)arg1;
- (void)cmdConnectTo:(id)arg1;
- (_Bool)validateConnectTo:(id)arg1;
- (void)cmdClearRecentFolders:(id)arg1;
- (_Bool)validateClearRecentFolders:(id)arg1;
- (void)cmdRecentFoldersItem:(id)arg1;
- (void)cmdRecentFolders:(id)arg1;
- (_Bool)validateRecentFolders:(id)arg1;
- (void)cmdSelectStartupDisk:(id)arg1;
- (_Bool)validateSelectStartupDisk:(id)arg1;
- (void)cmdToggleViewOptions:(id)arg1;
- (_Bool)validateToggleViewOptions:(id)arg1;
- (void)cmdShowClipboard:(id)arg1;
- (void)redo:(id)arg1;
- (_Bool)validateRedo:(id)arg1;
- (void)undo:(id)arg1;
- (_Bool)validateUndo:(id)arg1;
- (void)cmdShowInspector:(id)arg1;
- (_Bool)validateShowInspector:(id)arg1;
- (void)cmdSetLabel:(id)arg1;
- (_Bool)validateSetLabel:(id)arg1;
- (_Bool)validateQuickLook:(id)arg1;
- (void)cmdEmptyTrashSilently:(id)arg1;
- (_Bool)validateEmptyTrashSilently:(id)arg1;
- (void)cmdEmptyTrash:(id)arg1;
- (_Bool)validateEmptyTrash:(id)arg1;
- (void)cmdCloseTab:(id)arg1;
- (_Bool)validateCloseTab:(id)arg1;
- (void)cmdSwitchToSimpleFinder:(id)arg1;
- (void)switchToSimpleFinder;
- (_Bool)validateSwitchToSimpleFinder:(id)arg1;
- (void)cmdSwitchToFullFinder:(id)arg1;
- (_Bool)validateSwitchToFullFinder:(id)arg1;
- (void)cmdPreferences:(id)arg1;
- (void)cmdAbout:(id)arg1;
- (void)releaseImageCache;
- (void)resetInternalCacheVersionForNode:(const struct TFENode *)arg1;
- (void)detachImageCacheFromView:(id)arg1;
- (void)attachImageCacheToView:(id)arg1;
- (BOOL)menuHasKeyEquivalent:(id)arg1 forEvent:(id)arg2 target:(id *)arg3 action:(SEL *)arg4;
- (void)menuNeedsUpdate:(id)arg1;
- (void)cmdShowHelp:(id)arg1;
- (void)updateHelpMenu;
- (void)resetEditMenuItemTitles;
- (void)buildRecentFoldersMenu:(id)arg1;
- (void)configureMenuBar;
- (void)loadMenuBar;
- (void)openAddressBookWithFinderKit;
- (void)openTextEditWithFinderKit;
- (void)debugFrameworksByLaunchingExecutable:(const struct TString *)arg1;
- (void)openSimpleFinderWindow;
- (void)authenticateForRaft;
- (void)runBuildbotRaftTest;
- (void)runRaftTest:(const struct TString *)arg1 optionalArgs:(const struct TString *)arg2 andQuit:(_Bool)arg3;
- (void)cmdToggleDSDiagnosticLogging:(id)arg1;
- (_Bool)validateToggleDSDiagnosticLogging:(id)arg1;
- (void)purgeInfoExtractorCache;
- (void)dumpInfoExtractorCache;
- (void)purgeThumbnailCache;
- (void)dumpThumbnailCache;
- (void)showTagLists;
- (void)resetTagList;
- (void)cmdTogglePref:(id)arg1;
- (_Bool)validateTogglePref:(id)arg1;
- (void)hideDebugMenu;
- (void)showDebugMenu;
- (void)cmdToggleDebugMenu:(id)arg1;
- (void)releaseDebugMenu;
- (void)animateToFullScreenFromDebugMenu;
- (void)dumpFrontWindowDataSource;
- (void)dumpActiveContainerChildren;
- (void)dumpFrontWindowTarget;
- (void)configureDebugMenu;
- (void)configureHelpMenu;
- (void)windowMenuDidSendAction:(id)arg1;
- (void)configureWindowMenu;
- (void)nodeObserver:(struct TNodeObserver *)arg1 nodeChanged:(const struct TFENode *)arg2 inObservedNode:(const struct TFENode *)arg3 property:(unsigned int)arg4;
- (void)updateGoToLibraryMenuItem;
- (void)configureGoMenu;
- (void)configureGoMenuIcons;
- (void)configureViewMenu;
- (void)configureEditMenu;
- (void)configureFileMenu;
- (void)configureNewFinderWindowMenuItem;
- (void)configureTagsMenuItems;
- (void)configureArrangeByMenuItem;
- (void)configureOpenWithMenuItem;
- (void)configureFinderMenu;
- (void)configureTrashMenuItems;
- (id)setMenuDelegateForItem:(long long)arg1 inMenu:(id)arg2;
- (BOOL)validateToolbarItem:(id)arg1;
- (BOOL)validateMenuItem:(id)arg1;
- (short)handleAction:(SEL)arg1;
- (BOOL)respondsToSelector:(SEL)arg1;
- (id)forwardingTargetForSelector:(SEL)arg1;
- (void)scheduleRemovingOldTrashItems;
- (void)dealloc;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

