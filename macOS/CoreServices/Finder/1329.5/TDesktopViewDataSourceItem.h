//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "NSCopying-Protocol.h"

@class NSArray, NSDictionary, NSImage, NSSet, NSString;

@interface TDesktopViewDataSourceItem : NSObject <NSCopying>
{
    struct TFENode _node;
    struct TNSRef<NSImage, void> _icon;
    struct TNSRef<NSSet<NSString *>, void> _itemInfoTextSet;
    _Bool _selected;
    _Bool _collapsed;
    shared_ptr_2bd9f2cf _group;
    _Bool _canBePiled;
    struct TNSRef<NSObject<NSCopying>, void> _sortValue;
    struct TString _displayNameForSort;
    struct TString _rawNameForSort;
    unsigned int _volumeIsMaskForSort;
    struct TICloudStateCoordinator _iCloudStateCoordinator;
    _Bool _animating;
    shared_ptr_2bd9f2cf _oldTagsGroup;
}

+ (id)keyPathsForValuesAffectingData;
- (id).cxx_construct;
- (void).cxx_destruct;
@property shared_ptr_2bd9f2cf oldTagsGroup; // @synthesize oldTagsGroup=_oldTagsGroup;
@property _Bool animating; // @synthesize animating=_animating;
@property(readonly) _Bool canBePiled; // @synthesize canBePiled=_canBePiled;
@property(getter=isSelected) _Bool selected; // @synthesize selected=_selected;
- (void)updateSortValuesWithNodeSortingData:(const struct TNodeSortingData *)arg1 sortBy:(int)arg2;
- (void)getSortValuesForSort:(struct TDesktopViewDataSourceItemSortValues *)arg1;
@property(readonly) _Bool shouldDisplayICloudBadge; // @dynamic shouldDisplayICloudBadge;
@property shared_ptr_2bd9f2cf group; // @dynamic group;
- (struct TNodeProgressStatus)inlineProgressStatus;
@property(readonly) _Bool itemShouldBeHidden;
- (_Bool)isCurrentPile;
@property(readonly, getter=isPileContainer) _Bool pileContainer;
@property(getter=isCollapsed) _Bool collapsed; // @synthesize collapsed=_collapsed;
@property(readonly, getter=isDimmed) _Bool dimmed; // @dynamic dimmed;
@property(readonly) NSString *itemInfoTextToolTip; // @dynamic itemInfoTextToolTip;
- (void)setItemInfoTextSetNoKVO:(id)arg1;
- (void)setItemInfoTextSet:(id)arg1;
@property(readonly) NSSet *itemInfoTextSet; // @dynamic itemInfoTextSet;
@property(readonly) NSArray *tagColorIndexes; // @dynamic tagColorIndexes;
@property(readonly) NSString *displayName; // @dynamic displayName;
- (void)checkForAnimatingToDestination;
- (void)setIconNoKVO:(id)arg1;
- (void)setIcon:(id)arg1;
@property(readonly) NSImage *icon; // @dynamic icon;
@property(readonly) NSDictionary *data; // @dynamic data;
- (const struct TFENode *)node;
- (BOOL)isEqual:(id)arg1;
- (unsigned long long)hash;
- (id)copyWithOriginalNodeForReplica;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)initWithNode:(const struct TFENode *)arg1 iconImage:(id)arg2 fetchIconAtSize:(unsigned long long)arg3;
- (id)initWithNode:(const struct TFENode *)arg1;

// Remaining properties
@property(readonly) _Bool inlineProgressCanCancel; // @dynamic inlineProgressCanCancel;
@property(readonly) unsigned long long inlineProgressPercentComplete; // @dynamic inlineProgressPercentComplete;
@property(readonly) unsigned int inlineProgressState; // @dynamic inlineProgressState;

@end

