//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "TBaseBrowserViewController.h"

#import "NSMenuDelegate-Protocol.h"
#import "QLPreviewMenuItemDelegate-Protocol.h"
#import "TDesktopViewDelegate-Protocol.h"

@class NSObject, NSString, TDesktopBandSelectionController, TDesktopGridLayoutController, TDesktopIconSpatialDataController, TDesktopInlinePreviewController, TDesktopViewDataSource;
@protocol TDesktopViewControllerDelegateProtocol;

@interface TDesktopViewController : TBaseBrowserViewController <NSMenuDelegate, TDesktopViewDelegate, QLPreviewMenuItemDelegate>
{
    struct TFENode _nodeClickedOnMouseDown;
    _Bool _deselectOnMouseUp;
    _Bool _doubleClickOnMouseUp;
    _Bool _startEditingOnMouseUp;
    _Bool _isSnappingToGrid;
    struct TFENode _pendingNodeToEdit;
    TNSRef_a11c280b _pendingRenameOp;
    struct TNSWeakPtr<NSObject<TDesktopViewControllerDelegateProtocol>, void> _weakDelegate;
    struct TKeyValueObserver _viewSettingsObserver;
    _Bool _didShowDesktopViaForceClick;
    TNSRef_469baef2 _pileIconClickedOnMouseDown;
    TNSRef_469baef2 _viewHiddenForSkimming;
    struct TNSRef<TDesktopViewDataSourceItem, void> _itemShownForSkimming;
    vector_9edeb3e0 _itemsToSkim;
    double _verticalSkimScrollAccumulation;
    double _horizontalSkimScrollAccumulation;
    _Bool _firstScrub;
    struct TNSRef<NSDate, void> _firstScrubAdvancement;
    _Bool _didEnterQuickLookViaForce;
    struct vector<TNotificationCenterObserver, std::__1::allocator<TNotificationCenterObserver>> _notificationCenterObservers;
    struct TNSRef<TBrowserImmediateActionGestureRecognizerDelegate, void> _immediateActionGestureRecognizerDelegate;
    struct TNSRef<NSImmediateActionGestureRecognizer, void> _quickLookImmediateActionGestureRecognizer;
    struct TNSRef<NSImmediateActionGestureRecognizer, void> _renameImmediateActionGestureRecognizer;
    _Bool _shouldSkipRename;
    _Bool _immediateActionGestureRecognizerDidRecognize;
    double _scaleFactorCache;
}

- (id).cxx_construct;
- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool immediateActionGestureRecognizerDidRecognize; // @synthesize immediateActionGestureRecognizerDidRecognize=_immediateActionGestureRecognizerDidRecognize;
@property double scaleFactorCache; // @synthesize scaleFactorCache=_scaleFactorCache;
- (void)_setUpImmediateActionGestureRecognizers;
- (BOOL)menuItem:(id)arg1 shouldPositionNearPreviewItemAtScreenPoint:(struct CGPoint)arg2;
- (void)cmdTogglePreviewOptions:(id)arg1;
- (_Bool)validateTogglePreviewOptions:(id)arg1;
- (void)cmdOpenParent:(id)arg1;
- (void)openSelectedNodes:(_Bool)arg1;
- (void)cmdOpenWithOptionDownArrow:(id)arg1;
- (void)cmdOpenWithDownArrow:(id)arg1;
- (void)cmdEditTags:(id)arg1;
- (void)cmdChangeDesktopBackground:(id)arg1;
- (_Bool)validateDeselectAll:(id)arg1;
- (_Bool)validateSelectAll:(id)arg1;
- (struct TriStateBool)includeTargetForCommandSelector:(SEL)arg1;
- (struct TFENode)nextNodeInViewAfter:(const struct TFENodeVector *)arg1;
- (struct TFENodeVector)typeSelectCandidateNodes;
- (void)renameSelectedItem;
- (void)quickLookWithEvent:(id)arg1;
- (void)scrollWheel:(id)arg1;
- (void)toggleSkimming:(id)arg1;
- (void)exitSkimming;
- (void)enterSkimming:(struct CGPoint)arg1;
- (void)hideIconForSkimming:(id)arg1;
- (void)showIconForSkimming:(id)arg1;
- (void)keyDown:(id)arg1;
- (void)mouseUp:(id)arg1;
- (_Bool)handleMouseDragged:(id)arg1;
- (_Bool)handleMouseDown:(id)arg1 outClickedNode:(struct TFENode *)arg2 outDeselectOnBandSelect:(_Bool *)arg3;
- (unsigned long long)getSelectedNodes:(struct TFENodeVector *)arg1 upTo:(unsigned long long)arg2;
- (unsigned long long)getCommandTarget:(SEL)arg1 outNodes:(struct TFENodeVector *)arg2 upTo:(unsigned long long)arg3;
- (int)filterOutPileNodes:(struct TFENodeVector *)arg1;
- (_Bool)handleRightMouseDown:(id)arg1;
- (_Bool)determineSnapToGridStateForDrag;
- (void)calculatePropertiesWithDrop:(struct TDropOperation *)arg1 forNode:(const struct TFENode *)arg2 withPasteboardItem:(void *)arg3 pbItemIndex:(unsigned long long)arg4 inPropertyList:(struct TFENodePropertyList *)arg5 forcePlace:(_Bool)arg6 applyNow:(_Bool)arg7;
- (id)dropTargetViewForDraggingInfo:(id)arg1;
- (void)receivePromiseDragNode:(const struct TFENode *)arg1 spatialData:(const struct IconSpatialData *)arg2;
- (void)calculateSpatialData:(struct IconSpatialData *)arg1 draggingItem:(id)arg2 forcePlace:(_Bool)arg3;
- (struct CGRect)handleDragFlockingFrameForNode:(const struct TFENode *)arg1 dropTargetView:(id)arg2;
- (struct CGRect)dragFlockingFrameForNode:(const struct TFENode *)arg1 dropTargetView:(id)arg2;
- (id)dragFlockingImageComponentsForNode:(const struct TFENode *)arg1 dropTargetView:(id)arg2;
- (_Bool)waitingForThumbnailForNode:(const struct TFENode *)arg1;
- (struct TFENode)nodeForDraggingItem:(id)arg1;
- (void)menuWillOpen:(id)arg1;
- (void)menu:(id)arg1 prepareForLocation:(struct CGPoint)arg2 inView:(id)arg3;
- (void)menuNeedsUpdate:(id)arg1;
- (id)accessibilityActionNames;
- (BOOL)accessibilityIsAttributeSettable:(id)arg1;
- (id)accessibilityAttributeValue:(id)arg1;
- (id)accessibilityAttributeNames;
- (BOOL)accessibilityIsIgnored;
- (unsigned long long)springLoadingUpdated:(id)arg1 dropTargetView:(id)arg2;
- (void)springLoadingActivated:(BOOL)arg1 draggingInfo:(id)arg2 dropTargetView:(id)arg3;
- (void)mouseExitedDesktopIcon:(id)arg1;
- (void)mouseEnteredDesktopIcon:(id)arg1;
- (void)concludeDragOperation:(id)arg1 dropTargetView:(id)arg2;
- (_Bool)performDragOperation:(id)arg1 dropTargetView:(id)arg2;
- (_Bool)prepareForDragOperation:(id)arg1 dropTargetView:(id)arg2;
- (_Bool)wantsPeriodicDraggingUpdatesWithDropTargetView:(id)arg1;
- (void)updateDraggingItemsForDrag:(id)arg1 dropTargetView:(id)arg2;
- (unsigned long long)draggingUpdated:(id)arg1 dropTargetView:(id)arg2;
- (id)quickLookSharedPreviewView;
- (id)zoomImageForNode:(const struct TFENode *)arg1 contentRect:(struct CGRect *)arg2;
- (id)popoverAnchorViewForNode:(const struct TFENode *)arg1;
- (struct CGRect)iconFrameForNode:(const struct TFENode *)arg1;
- (struct CGRect)frameForNode:(const struct TFENode *)arg1;
- (void)shrinkToFitTextViewAboutToCloseForNode:(const struct TFENode *)arg1;
- (void)shrinkToFitTextViewEditingComplete:(id)arg1 forNode:(const struct TFENode *)arg2 renameOp:(id)arg3;
- (_Bool)shrinkToFitTextViewAboutToOpenForNode:(const struct TFENode *)arg1;
- (void)stopEditing:(_Bool)arg1;
- (_Bool)startEditingWithNode:(const struct TFENode *)arg1 renameOp:(id)arg2 afterDelay:(_Bool)arg3;
- (_Bool)startEditingWithNode:(const struct TFENode *)arg1 renameOp:(id)arg2;
- (_Bool)startEditingNodeNowOrLater:(const struct TFENode *)arg1 renameOp:(id)arg2;
- (void)iCloudBadgeButtonPressed:(const struct TFENode *)arg1;
- (void)quickLookPreviewInvoked:(const struct TFENode *)arg1;
- (void)inlineProgressCancelButtonPressed:(const struct TFENode *)arg1;
- (int)inlinePreviewIconFlavorForNode:(const struct TFENode *)arg1;
- (struct CGRect)inlinePreviewContentFrameForNode:(const struct TFENode *)arg1;
- (id)inlinePreviewPropertiesForNode:(const struct TFENode *)arg1;
- (void)setProperties:(const struct TFENodePropertyList *)arg1 onArchiveNode:(const struct TFENode *)arg2;
- (_Bool)useSpecialInFlightAnimation;
- (void)addOffsetSpatialDataFromNodes:(const struct TFENodeVector *)arg1 toPropertyList:(vector_d50fdf79 *)arg2;
- (void)addSpatialDataForLocationInView:(const struct CGPoint *)arg1 toPropertyList:(struct TFENodePropertyList *)arg2;
- (struct TFENode)nodeAtPoint:(const struct CGPoint *)arg1 includeTarget:(_Bool)arg2;
- (void)selectNodesNowOrLater:(const struct TFENodeVector *)arg1;
- (void)revealNodes:(const struct TFENodeVector *)arg1 select:(_Bool)arg2;
- (void)deselectAllNodes;
- (void)selectAllNodes;
- (void)setSelectedNodes:(const struct TFENodeVector *)arg1 byExtendingSelection:(_Bool)arg2;
- (void)updateOrderedSelection;
- (unsigned long long)getSelectedNodesFromView:(struct TFENodeVector *)arg1 upTo:(unsigned long long)arg2;
@property(nonatomic, getter=isSnapToGrid) _Bool snapToGrid; // @dynamic snapToGrid;
@property(nonatomic, getter=isKeepArranged) _Bool keepArranged; // @dynamic keepArranged;
@property(nonatomic, getter=isTitleOnBottom) _Bool titleOnBottom; // @dynamic titleOnBottom;
@property(nonatomic) _Bool showItemInfo; // @dynamic showItemInfo;
@property(nonatomic) double gridSpacing; // @dynamic gridSpacing;
- (void)setTextSize:(double)arg1;
- (double)textSize;
- (void)setIconSize:(double)arg1;
- (double)iconSize;
- (_Bool)canHaveRedundantReplicaNodes;
- (void)reloadIconsInView;
- (void)browserViewEffectiveAppearanceChanged;
- (void)updateDataSourceDarkBackground;
- (void)browserViewBackingPropertiesChanged;
- (_Bool)groupingByTag;
- (_Bool)isGrouping;
- (void)prepareSnapToGridForNode:(const struct TFENode *)arg1;
- (void)cancelDelayedNodeEventHandling;
- (void)delayNodeEventHandling:(duration_3c68f186)arg1;
- (void)flushNodeEvents;
- (unsigned long long)itemCount;
- (id)originatingWindowController;
- (void)windowDidBecomeMain:(id)arg1;
- (id)desktopView;
@property(nonatomic) __weak NSObject<TDesktopViewControllerDelegateProtocol> *delegate; // @dynamic delegate;
@property(nonatomic) __weak TDesktopInlinePreviewController *inlinePreviewController; // @dynamic inlinePreviewController;
@property(nonatomic) __weak TDesktopBandSelectionController *bandSelectionController; // @dynamic bandSelectionController;
@property(nonatomic) __weak TDesktopIconSpatialDataController *spatialDataController; // @dynamic spatialDataController;
@property(nonatomic) __weak TDesktopGridLayoutController *gridLayoutController; // @dynamic gridLayoutController;
@property(nonatomic) __weak TDesktopViewDataSource *dataSource; // @dynamic dataSource;
- (_Bool)isDesktopBrowser;
- (const struct TFENodeVector *)resolvedTargetPath;
- (const struct TFENode *)target;
- (void)aboutToTearDown;
- (void)configureView;
- (void)initCommon;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

