//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Cocoa/NSMutableString.h>

@interface NSMutableString (JensAdditions)
- (void)replaceSubstring:(id)arg1 with:(id)arg2;
- (void)searchAndReplaceInString:(id)arg1 withString:(id)arg2;
- (void)replaceOccurrencesOfString:(id)arg1 withString:(id)arg2;
- (void)searchAndReplaceSet:(id)arg1 withString:(id)arg2;
- (void)appendUnsignedChar:(unsigned char)arg1;
- (void)deleteCharactersFromSet:(id)arg1;
@end

