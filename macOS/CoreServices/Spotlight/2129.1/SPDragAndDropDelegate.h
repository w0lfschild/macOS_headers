//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import "NSFilePromiseProviderDelegate-Protocol.h"
#import "SearchUIDragAndDropDelegate-Protocol.h"

@class NSString;

@interface SPDragAndDropDelegate : NSObject <NSFilePromiseProviderDelegate, SearchUIDragAndDropDelegate>
{
}

+ (id)sharedDradAndDropSupport;
- (id)operationQueueForFilePromiseProvider:(id)arg1;
- (void)filePromiseProvider:(id)arg1 writePromiseToURL:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;
- (id)filePromiseProvider:(id)arg1 fileNameForType:(id)arg2;
- (id)tableView:(id)arg1 pasteboardWriterForResult:(id)arg2;
- (id)draggedTypes;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

