//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <swiftCore/_TtCs12_SwiftObject.h>

@interface _TtC10RealityKit5Scene : _TtCs12_SwiftObject
{
    // Error parsing type: , name: coreScene
    // Error parsing type: , name: __audioListener
    // Error parsing type: , name: internalAudioListener
    // Error parsing type: , name: defaultCamera
    // Error parsing type: , name: activeCamera
    // Error parsing type: , name: entities
    // Error parsing type: , name: $__lazy_storage_$_eventService
    // Error parsing type: , name: $__lazy_storage_$___interactionService
    // Error parsing type: , name: synchronizationService
}

@end

