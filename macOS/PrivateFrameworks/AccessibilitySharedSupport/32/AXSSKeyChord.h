//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCopying.h"
#import "NSSecureCoding.h"

@class NSArray, NSNumber, NSString;

@interface AXSSKeyChord : NSObject <NSSecureCoding, NSCopying>
{
    NSNumber *_cachedIsFunctionKey;
    NSNumber *_cachedContainsModifier;
    NSArray *_keys;
    NSArray *_backupKeys;
}

+ (BOOL)supportsSecureCoding;
+ (id)keyFromKeyCode:(unsigned long long)arg1 unicodeCharacter:(id)arg2;
+ (id)nullKeyChord;
+ (id)keyChordWithString:(id)arg1;
+ (id)keyChordWithKeys:(id)arg1;
+ (id)keyChordWithKeys:(id)arg1 backupKeys:(id)arg2;
- (void).cxx_destruct;
@property(retain, nonatomic) NSArray *backupKeys; // @synthesize backupKeys=_backupKeys;
@property(retain, nonatomic) NSArray *keys; // @synthesize keys=_keys;
- (id)_displayValueWithSortedModifiers:(id)arg1 isUSKeyboard:(BOOL)arg2;
- (id)displayValueWithStyle:(long long)arg1 isUSKeyboard:(BOOL)arg2;
- (id)displayValueWithStyle:(long long)arg1;
@property(readonly, nonatomic) NSString *displayValue;
@property(readonly, nonatomic) BOOL isNull;
@property(readonly, nonatomic) BOOL isTextInputTabChord;
@property(readonly, nonatomic) BOOL isArrowKeyChord;
@property(readonly, nonatomic) BOOL isTextInputChord;
@property(readonly, nonatomic) BOOL containsModifier;
@property(readonly, nonatomic) BOOL isFunctionKeyChord;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (unsigned long long)hash;
- (BOOL)isEqualToKeyChord:(id)arg1;
- (BOOL)isEqual:(id)arg1;
- (id)description;
- (void)encodeWithCoder:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (id)_normalizeKeys:(id)arg1;
- (id)_initWithKeys:(id)arg1 backupKeys:(id)arg2;
- (id)_initWithKeys:(id)arg1;

@end

