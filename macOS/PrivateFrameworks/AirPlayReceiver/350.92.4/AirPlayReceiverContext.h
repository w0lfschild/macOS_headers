...
{
    // Error parsing type: ^{AirPlayReceiverSessionPrivate={__CFRuntimeBase=QAQ}@^{AirPlayReceiverServerPrivate}^{OpaqueAPReceiverRequestProcessor}^{APReceiverSessionManagerOpaque}^v{?=^v^v^?^?^?^?^?^?}[32c][17c]^{OpaqueAPReceiverStatsCollector}@I(?={sockaddr=CC[14c]}{sockaddr_in=CCS{in_addr=I}[8c]}{sockaddr_in6=CCSI{in6_addr=(?=[16C][8S][4I])}I})[16C]iQ[6C]CC^{__CFString}^{__CFString}QIQQI{?=^{_CCCryptor}[16C]}^{?}[16C][16C]CCC^{OpaqueAPSNetworkClock}CCC^{HTTPClientPrivate}ii^{APPairingServicesPrivate}IIQQQQ^{APReceiverAudioSessionPrivate}I^{OpaqueFigValeria}I^{OpaqueAPReceiverScreenSession}dCCIQCCQ^{__CFDictionary}i}, name: _session
