//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <EmailDaemon/NSObject-Protocol.h>

@class EFQuery, EMContentRequestOptions, EMMailboxScope, EMMessageChangeAction, EMMessageObjectID, EMObjectID, NSArray, NSProgress;
@protocol EMContentItemRequestDelegate, EMMessageListItemQueryResultsObserver, EMMessageRepositoryCountQueryObserver_xpc;

@protocol EMMessageRepositoryInterface <NSObject>
- (void)resetPrecomputedThreadScopesForMailboxScope:(EMMailboxScope *)arg1;
- (NSProgress *)requestRepresentationForMessageWithID:(EMMessageObjectID *)arg1 options:(EMContentRequestOptions *)arg2 delegate:(id <EMContentItemRequestDelegate>)arg3 completionHandler:(void (^)(id <_EMDistantContentRepresentation>, EMContentRepresentation *, NSError *))arg4;
- (void)performMessageChangeAction:(EMMessageChangeAction *)arg1 returnUndoAction:(BOOL)arg2 completionHandler:(void (^)(EMUndoMessageAction *))arg3;
- (void)messageListItemsForObjectIDs:(NSArray *)arg1 loadSummaryForAdditionalObjectIDs:(NSArray *)arg2 completionHandler:(void (^)(NSArray *, NSDictionary *))arg3;
- (void)startCountingQuery:(EFQuery *)arg1 includingServerCountsForMailboxScope:(EMMailboxScope *)arg2 withObserver:(id <EMMessageRepositoryCountQueryObserver_xpc>)arg3 completionHandler:(void (^)(id <EFCancelable>))arg4;
- (void)performQuery:(EFQuery *)arg1 withObserver:(id <EMMessageListItemQueryResultsObserver>)arg2 observationIdentifier:(EMObjectID *)arg3 completionHandler:(void (^)(id <EFCancelable>))arg4;
- (void)performCountQuery:(EFQuery *)arg1 completionHandler:(void (^)(NSNumber *, NSError *))arg2;
- (void)performQuery:(EFQuery *)arg1 limit:(long long)arg2 completionHandler:(void (^)(NSArray *, NSError *))arg3;
@end

