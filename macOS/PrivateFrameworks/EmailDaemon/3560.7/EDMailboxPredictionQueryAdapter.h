//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <EmailDaemon/EDMailboxPredictionQueryAdapter-Protocol.h>

@class EDMailboxPersistence, EDMessagePersistence, NSString;

@interface EDMailboxPredictionQueryAdapter : NSObject <EDMailboxPredictionQueryAdapter>
{
    EDMessagePersistence *_messagePersistence;
    EDMailboxPersistence *_mailboxPersistence;
}

@property(readonly, nonatomic) EDMailboxPersistence *mailboxPersistence; // @synthesize mailboxPersistence=_mailboxPersistence;
@property(readonly, nonatomic) EDMessagePersistence *messagePersistence; // @synthesize messagePersistence=_messagePersistence;
- (void).cxx_destruct;
- (id)groupedMessagesCountByMailboxMatchingQuery:(unsigned long long)arg1 variable:(id)arg2;
- (id)validMailboxesForPrediction;
- (id)_userCreatedMailboxes;
- (id)initWithMessagePersistence:(id)arg1 mailboxPersistence:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

