//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <PhotoLibraryPrivate/RDTable.h>

@class LiKeyPath;

@interface RDFaceExternalTable : RDTable
{
    LiKeyPath *_faceKeyKeyPath;
    LiKeyPath *_serviceKeyKeyPath;
    LiKeyPath *_referenceKeyPath;
}

+ (id)tableClassName;
+ (id)tableName;
+ (id)modelName;
+ (Class)modelClass;
@property(readonly, nonatomic) LiKeyPath *referenceKeyPath; // @synthesize referenceKeyPath=_referenceKeyPath;
@property(readonly, nonatomic) LiKeyPath *serviceKeyKeyPath; // @synthesize serviceKeyKeyPath=_serviceKeyKeyPath;
@property(readonly, nonatomic) LiKeyPath *faceKeyKeyPath; // @synthesize faceKeyKeyPath=_faceKeyKeyPath;
- (void).cxx_destruct;
- (void)declareProperties;
- (id)initWithTableId:(unsigned long long)arg1 tableType:(unsigned char)arg2 withLibrary:(id)arg3;
- (id)notifyUpdateKeyPaths;
- (unsigned long long *)faultedKeyPath;
- (unsigned long long *)faultedModelKeyPath;

@end

