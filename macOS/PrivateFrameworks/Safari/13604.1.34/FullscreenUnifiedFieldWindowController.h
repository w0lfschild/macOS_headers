//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <AppKit/NSController.h>

#import <Safari/UnifiedFieldDelegate-Protocol.h>
#import <Safari/UnifiedFieldSecurityUIManagerDelegate-Protocol.h>

@class BrowserContentViewControllerObjCAdapter, NSString, NSWindow, PopupWindowUnifiedField, PopupWindowUnifiedFieldViewController, UnifiedFieldSecurityUIManager;

__attribute__((visibility("hidden")))
@interface FullscreenUnifiedFieldWindowController : NSController <UnifiedFieldDelegate, UnifiedFieldSecurityUIManagerDelegate>
{
    BrowserContentViewControllerObjCAdapter *_browserContentViewControllerObjCAdapter;
    PopupWindowUnifiedFieldViewController *_unifiedFieldViewController;
    NSWindow *_window;
    UnifiedFieldSecurityUIManager *_unifiedFieldSecurityUIManager;
    CDUnknownBlockType _restoreWindowCallback;
    PopupWindowUnifiedField *_unifiedField;
}

@property(readonly, nonatomic) PopupWindowUnifiedField *unifiedField; // @synthesize unifiedField=_unifiedField;
- (void).cxx_destruct;
- (void)unifiedFieldSecurityUIManagerDidEndShowingCertificateSheet:(id)arg1;
- (id)unifiedFieldFallbackURLForAboutBlank:(id)arg1;
- (void)unifiedFieldPerSitePreferencesContextMenuItemWasSelected:(id)arg1;
- (BOOL)unifiedFieldCanShowPerSitePreferences:(id)arg1;
- (void)unifiedFieldDidChangePageTitle:(id)arg1;
- (void)unifiedFieldURLDropped:(id)arg1;
- (id)bookmarkToDragFromSiteIconInUnifiedField:(id)arg1;
- (BOOL)unifiedField:(id)arg1 trackSearchButtonForEvent:(id)arg2;
- (id)unifiedFieldReloadMenu:(id)arg1;
- (id)unifiedFieldReaderMenu:(id)arg1;
- (id)unifiedFieldAudioIndicatorMenu:(id)arg1;
- (void)unifiedFieldPerformAudioIndicatorAction:(id)arg1;
- (void)unifiedFieldPerformSecurityPillAction:(id)arg1;
- (void)unifiedFieldPerformRightmostButtonAction:(id)arg1;
- (void)unifiedFieldWasClicked:(id)arg1;
- (BOOL)unifiedFieldShouldSelectAllAfterMouseDown:(id)arg1;
- (void)unifiedFieldMarkedTextDidChange:(id)arg1;
- (void)unifiedFieldAutocompleteSelectionDidChange:(id)arg1;
- (void)unifiedFieldResignedFirstResponder:(id)arg1;
- (void)unifiedFieldBecameFirstResponder:(id)arg1;
- (void)unifiedFieldDidMoveToWindow:(id)arg1;
- (void)dealloc;
- (void)_updateUnifiedFieldText;
- (void)_setUpPopupWindow;
- (id)initWithCoder:(id)arg1;
- (id)init;
- (id)initWithBrowserContentViewControllerObjCAdapter:(id)arg1 fullscreenWindow:(id)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

