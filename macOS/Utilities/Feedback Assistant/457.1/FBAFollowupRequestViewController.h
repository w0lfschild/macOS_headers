//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 25 2017 03:49:04).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <AppKit/NSViewController.h>

#import "FBACorrespondingViewController-Protocol.h"

@class FBAFeedbackFollowup, NSAttributedString, NSButton, NSDate, NSImage, NSNumber, NSString, NSTextField;

@interface FBAFollowupRequestViewController : NSViewController <FBACorrespondingViewController>
{
    FBAFeedbackFollowup *_followup;
    NSButton *_replyToAppleFeedbackButton;
    NSTextField *_requirementsField;
}

+ (id)keyPathsForValuesAffectingRequirementsText;
+ (id)keyPathsForValuesAffectingAttributedDisplayText;
+ (id)keyPathsForValuesAffectingDisplayText;
+ (id)keyPathsForValuesAffectingCorrespondenceUnread;
- (void).cxx_destruct;
@property __weak NSTextField *requirementsField; // @synthesize requirementsField=_requirementsField;
@property __weak NSButton *replyToAppleFeedbackButton; // @synthesize replyToAppleFeedbackButton=_replyToAppleFeedbackButton;
@property(retain) FBAFeedbackFollowup *followup; // @synthesize followup=_followup;
- (void)awakeFromNib;
- (void)replyToAppleFeedback:(id)arg1;
- (id)initWithFollowup:(id)arg1;
@property(readonly) NSString *requirementsText;
- (id)attributedDisplayText;
@property(readonly) NSString *displayText;
@property(readonly) BOOL correspondenceUnread;
@property(readonly) NSImage *correspondenceImage;
@property(readonly) NSDate *correspondenceTime;
@property(readonly) NSString *correspondenceTitle;
- (void)loadView;

// Remaining properties
@property(readonly) NSNumber *correspondenceOwnerStatus;
@property(readonly) NSAttributedString *correspondenceStatus;
@property(readonly) NSString *correspondenceSubtitle;
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

